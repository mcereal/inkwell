 ...
 * nothing on its own - it answers how many bytes came out, and inkwell_crc32() answers whether they
 * are the right ones. Both halves are Wuffs (third_party/wuffs-config/inkwell_wuffs.h), which
 * codec/png.c already compiles in - a deflate decoder and a CRC32 are what a PNG is made of, so
 * this costs no module that was not already there.
 *
 * Whole buffers rather than a stream, because the one caller has the member in memory and knows
 * the size it must produce - a zip's central directory states the uncompressed length, and the
 * destination is allocated at exactly that. A streaming decoder is a different API with a
 * different failure model, and the caller that needs one should ask for it rather than get it
 * by default.
 */
...
enum inkwell_inflate_result {
    INKWELL_INFLATE_OK = 0,
    ...
    INKWELL_INFLATE_TOO_LONG,
    INKWELL_INFLATE_NO_MEMORY,
};
