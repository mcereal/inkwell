...
#define INKWELL_HTTP_HEAD_MAX 16384U
/* A URL, and so a `Location` - the signed release-asset redirects are the long ones. */
#define INKWELL_HTTP_URL_MAX 4096U
#define INKWELL_HTTP_HOST_MAX 256U

/* ------------------------------------------------------------------ URLs */

struct inkwell_http_url {
    bool tls; /* https */
    /* No brackets, even for a v6 literal; inkwell_http_request_format() puts them back. */
    char host[INKWELL_HTTP_HOST_MAX];
    uint16_t port;
    /* The request target: path and query, always starting with '/'. No fragment. */
    char target[INKWELL_HTTP_URL_MAX];
};
...
enum inkwell_http_framing {
    INKWELL_HTTP_FRAMING_NONE = 0, /* no body: a HEAD, a 204, a 304 */
    INKWELL_HTTP_FRAMING_LENGTH,   /* Content-Length */
    INKWELL_HTTP_FRAMING_CHUNKED,  /* Transfer-Encoding: chunked */
    /*
     ...
     */
    INKWELL_HTTP_FRAMING_UNTIL_CLOSE,
};

/* Why a response was refused. Each names what was wrong with the bytes, not what to do. */
enum inkwell_http_error {
    INKWELL_HTTP_OK = 0,
    INKWELL_HTTP_ERR_STATUS_LINE, /* not `HTTP/1.x NNN ...` */
    INKWELL_HTTP_ERR_HEADER,      /* a header line that is not `name: value`, or is folded */
    INKWELL_HTTP_ERR_HEAD_TOO_LARGE,
    INKWELL_HTTP_ERR_LENGTH,    /* a Content-Length that is not a number, or two that disagree */
    INKWELL_HTTP_ERR_ENCODING,  /* a Transfer-Encoding other than `chunked` */
    INKWELL_HTTP_ERR_CHUNK,     /* a chunk size line or chunk terminator that is not one */
    INKWELL_HTTP_ERR_UPGRADE,   /* a 101: this never asks to switch protocols */
    INKWELL_HTTP_ERR_TRUNCATED, /* the connection ended before the body did */
    INKWELL_HTTP_ERROR_COUNT,
};

struct inkwell_http_response {
    /* ---- the answer, valid once inkwell_http_response_head_done() */
    int status;
    enum inkwell_http_framing framing;
    ...
    char head[INKWELL_HTTP_HEAD_MAX];
    size_t head_len;
