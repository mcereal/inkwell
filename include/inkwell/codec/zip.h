 ...
 *
 *   1. the tail       -> inkwell_zip_find_end()          where the central directory is
 *   2. the directory  -> inkwell_zip_find_member()       the member's sizes, CRC and where it is
 *   3. a local header -> inkwell_zip_local_data_start()  where its bytes actually begin
 *
 ...
 * gigabytes. inkwell_zip_find_end() refuses them instead.
 */
 ...
 * directory is in the window we already hold is a question inkwell_zip_central_slice() answers
 ...
 */
#define INKWELL_ZIP_TAIL_WINDOW 65536U

/* The fixed part of a local file header, ahead of its name and extra field. */
#define INKWELL_ZIP_LOCAL_HEADER_SIZE 30U
 ...
 */
#define INKWELL_ZIP_NAME_MAX 96U
...
struct inkwell_zip_entry {
    char name[INKWELL_ZIP_NAME_MAX];
    uint16_t flags;
    uint16_t method;
    uint32_t crc32;
 ...
 */
bool inkwell_zip_find_end(const uint8_t *window, size_t len, uint64_t window_offset,
                          struct inkwell_zip_end *out);
 ...
 * inkwell_zip_find_member() on those instead" - not an error. Both zips this was written against
 ...
 */
const uint8_t *inkwell_zip_central_slice(const struct inkwell_zip_end *end, const uint8_t *window,
 ...
 */
enum inkwell_zip_search inkwell_zip_find_member(const uint8_t *central, size_t len,
                                                uint32_t entries, const char *basename,
                                                struct inkwell_zip_entry *out);

/*
 * Where `entry`'s compressed bytes begin, given the fixed part of its local header.
 *
 * The caller range-reads INKWELL_ZIP_LOCAL_HEADER_SIZE bytes at `entry->local_header_offset` and
 ...
 */
bool inkwell_zip_local_data_start(const uint8_t *header, size_t len,
                                  const struct inkwell_zip_entry *entry, uint64_t *out_offset);
