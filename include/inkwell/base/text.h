...
/*
 * UTF-8 helpers shared by everything that touches text from outside the process.
 *
 * Names and message bodies are chosen by whoever owns the machine at the other end, so they are
 * untrusted bytes that end up in a framebuffer, a log line and a JSON document. A device
 * protocol's own conventions often make multi-byte text the norm rather than the exception: a
 * short display name sized `char[5]` holds exactly one four-byte emoji plus its NUL.
 */

/* What a decoder yields for a byte that is not part of a well-formed sequence. */
#define INKWELL_TEXT_INVALID_CODEPOINT 0xFFFDU
 ...
 * copied through into one would produce a document that standards-compliant parsers reject -
 * and the bytes come from off the device, where anyone can choose them.
 */
size_t inkwell_text_utf8_sequence_len(const uint8_t *bytes, size_t available);
 ...
 * A malformed byte consumes one byte and yields INKWELL_TEXT_INVALID_CODEPOINT, so a caller that
 * loops on the return value always makes progress.
 */
size_t inkwell_text_utf8_next(const char *text, uint32_t *codepoint);
...
size_t inkwell_text_utf8_length(const char *text);
...
size_t inkwell_text_utf8_offset(const char *text, size_t index);

/* Truncate `text` in place to at most `max_chars` characters. */
void inkwell_text_utf8_truncate(char *text, size_t max_chars);
 ...
 */
void inkwell_text_sanitise(const uint8_t *payload, size_t len, char *out, size_t out_len);

/* inkwell_text_sanitise for a NUL-terminated source. `in` may be NULL, which yields "". */
void inkwell_text_sanitise_str(const char *in, char *out, size_t out_len);
 ...
 *
 * Truncation cuts at a byte, not a character. Callers rendering UTF-8 want inkwell_text_sanitise()
 * or a inkwell_text_utf8_truncate() afterwards.
 */
bool inkwell_str_copy(char *out, size_t out_len, const char *in);
