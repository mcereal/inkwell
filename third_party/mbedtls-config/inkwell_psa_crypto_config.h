 ...
 * `inkwell_mbedtls_config.h` does. Read that file's opening comment first; it explains why this
 ...
 *
 * This file is ours; `third_party/mbedtls` next to it is the submodule and is never edited.
 */

/* ---- primitives no public server offers -------------------------------------------------- */

/*
 * Block ciphers that are not AES. Camellia and ARIA are regional alternatives that no public
 * server has ever negotiated.
 *
...
/* Built-in test vectors for every primitive, which nothing here calls. */
#undef MBEDTLS_SELF_TEST

/* ---- what is deliberately kept ----------------------------------------------------------- */

/*
 * `MBEDTLS_FS_IO` stays because a CA bundle may be a file - `inkwell_tls_client_start()` takes a
 * path, for the device with no system certificate store whose application ships its own. It is
 * configured on this side of the split in 4.x, although the thing that needs it -
 * `mbedtls_x509_crt_parse_file()` - is on the other.
 */
 ...
 * when one was not available both at compile time and at run time - and a cross toolchain
 * targeting an older kernel does not satisfy that. So such a build takes the file path, and the
 * file upstream picks by default is `/dev/random`.
 *
 * On a desktop that is merely old-fashioned. On a small handheld it is a hang. `/dev/random`
 * blocks until the kernel's entropy *estimate* reaches read_wakeup_threshold (64 bits on the
 * device this was measured on), and a machine with no disks, no network interrupts worth the
 * name and no hardware RNG refills that estimate at a crawl: measured at 15 bits, climbing to 54
 * over three minutes, with a TLS handshake waiting on the other side of it.
 *
 * And the wait is not in a corner. `inkwell_tls_client_start()` calls `psa_crypto_init()` inline,
 * on the one loop thread, and that is what seeds the PSA RNG in 4.x - so everything stops: no
 * frames, no input handling, not even the press that quits. It presents as a frozen device, and
 * the only thing on screen that could explain it is a status display that is no longer being
 * drawn.
 *
 * (In 3.x the inline seed was the TLS config's own `mbedtls_ctr_drbg_seed()` call. 4.0 removed
 * the DRBG plumbing from the TLS config and the wait moved into `psa_crypto_init()`. Same
 * thread, same stall, one frame earlier.)
 *
