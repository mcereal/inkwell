 ...
 * that works against the server the author tested and fails against somebody else's with a
 * cipher suite mismatch nobody can read. Starting from the library's own tested default and
 * removing only what this can provably never meet keeps that class of mistake out.
 *
 ...
 * curves, where randomness comes from - is configured next door in `inkwell_psa_crypto_config.h`,
...
/* ---- protocols this does not speak ------------------------------------------------------- */

/*
 * No DTLS. `net/tls.h` is a session on a stream descriptor, and datagram TLS brings its own
 * retransmission timer, replay window and cookie exchange - a second, quite different state
 * machine that nothing here would ever enter.
 */
 ...
 * certificate against a set of trust anchors, and that is the whole of the trust model this
 * offers: a PSK suite would be a second one with no way to configure it, since nothing in
 * `inkwell_tls_client_start()` takes a key.
 *
 ...
 * project cannot use: there is one epoll loop and nothing may block on it, so src/net/tls.c
 * supplies non-blocking BIO callbacks over a descriptor the caller owns. Turning these off is
 * not a size optimisation - it is making the unusable thing unavailable, so a later change
 * cannot reach for `mbedtls_net_connect()` and quietly stall every other source on the loop.
 */
#undef MBEDTLS_NET_C
 ...
 * does is that "why is this thing not reaching that server" has to be answerable where there is
 * no terminal attached. A numeric -0x2700 on a handheld is not an answer.
 *
 * `MBEDTLS_SSL_SRV_C` stays, although nothing here is ever a server. It is what lets
 * tests/support/https_fixture.c stand a real TLS server on a loopback socket and complete a real
