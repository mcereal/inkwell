...
size_t inkwell_text_utf8_next(const char *text, uint32_t *codepoint) {
    ...
    const size_t length = inkwell_text_utf8_sequence_len(bytes, available);
    if (length == 0U) {
        /* Consume exactly one byte so a caller looping on the return value cannot stall. */
        if (codepoint != NULL) {
            *codepoint = INKWELL_TEXT_INVALID_CODEPOINT;
        }
        return 1U;
    }
...
size_t inkwell_text_utf8_length(const char *text) {
    ...
    size_t offset = 0;
    for (;;) {
        const size_t step = inkwell_text_utf8_next(&text[offset], NULL);
        if (step == 0U) {
            break;
        }
        offset += step;
...
void inkwell_text_utf8_truncate(char *text, size_t max_chars) {
    ...
    const size_t offset = inkwell_text_utf8_offset(text, max_chars);
    text[offset] = '\0';
}
...
void inkwell_text_sanitise(const uint8_t *payload, size_t len, char *out, size_t out_len) {
    ...
    while (i < len) {
        ...
        const size_t sequence = inkwell_text_utf8_sequence_len(&payload[i], len - i);
        if (sequence == 0U) {
            if (written + 1U > out_len - 1U) {
                break;
            }
...
void inkwell_text_sanitise_str(const char *in, char *out, size_t out_len) {
    ...
    inkwell_text_sanitise((const uint8_t *)in, strlen(in), out, out_len);
}
