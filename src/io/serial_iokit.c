...
static bool device_has_mass_storage(io_registry_entry_t device) {
    ...
    while (!found && (child = IOIteratorNext(it)) != 0) {
        ...
        if (own_long(child, CFSTR("bInterfaceClass"), &cls) &&
            cls == (long)INKWELL_USB_CLASS_MASS_STORAGE &&
            own_long(child, CFSTR("bInterfaceSubClass"), &sub) &&
            sub == (long)INKWELL_USB_SUBCLASS_SCSI &&
            own_long(child, CFSTR("bInterfaceProtocol"), &proto) &&
            proto == (long)INKWELL_USB_PROTOCOL_BULK_ONLY) {
            found = true;
        }
...
size_t inkwell_serial_scan_iokit(struct inkwell_serial_port_info *out, size_t capacity) {
    ...
    while (count < capacity && (port = IOIteratorNext(it)) != 0) {
        ...
        /* The nearest bInterfaceClass above the tty is the interface it was made from: CDC-Data
           for a native device, vendor-specific for a bridge chip. */
        long iface_class = -1;
        const bool cdc_data = cf_to_long(search_up(port, CFSTR("bInterfaceClass")), &iface_class) &&
                              iface_class == (long)INKWELL_USB_CLASS_CDC_DATA;
        info->kind = cdc_data ? INKWELL_SERIAL_NATIVE : INKWELL_SERIAL_BRIDGE;
        info->mass_storage = cdc_data && device_has_mass_storage(device);
