 ...
 *
 * A busy broker can hand over data faster than this can be woken to ask
...
static void mqtt_attempt(struct inkwell_mqtt_client *proxy);
 ...
 * anything.
 */
static bool mqtt_has_failure(const struct inkwell_mqtt_client *proxy) {
    return inkwell_net_failed(&proxy->failure.net) ||
           proxy->failure.refusal != INKWELL_MQTT_REFUSAL_NONE;
}
...
static const char *mqtt_refusal_name(enum inkwell_mqtt_refusal refusal) {
    switch (refusal) {
 ...
 */
static void mqtt_back_off(struct inkwell_mqtt_client *proxy, const char *why, const char *detail) {
    mqtt_close(proxy);
    ...
    const unsigned shift =
        proxy->failures > 1U
            ? (unsigned)(proxy->failures - 1U > MQTT_BACKOFF_SHIFT_MAX ? MQTT_BACKOFF_SHIFT_MAX
                                                                       : proxy->failures - 1U)
            : 0U;
    uint64_t delay = (uint64_t)INKWELL_MQTT_CLIENT_BACKOFF_BASE_MS << shift;
    if (delay > INKWELL_MQTT_CLIENT_BACKOFF_MAX_MS) {
...
static void mqtt_fail_net(struct inkwell_mqtt_client *proxy, enum inkwell_net_reason reason,
                          int detail) {
...
static void mqtt_fail_own(struct inkwell_mqtt_client *proxy, enum inkwell_mqtt_refusal refusal,
                          uint8_t code) {
...
static void mqtt_compact_out(struct inkwell_mqtt_client *proxy) {
 ...
 */
static int mqtt_flush(struct inkwell_mqtt_client *proxy) {
     ...
     */
    if (sent_something) {
        proxy->next_ping_ms = proxy->now_ms + INKWELL_MQTT_CLIENT_PING_INTERVAL_MS;
    }
    mqtt_arm(proxy);
    return 0;
}
...
static int mqtt_queue(struct inkwell_mqtt_client *proxy, const uint8_t *data, size_t len) {
 ...
 * and the thing a refusal has to say is *which topic*, since that is the filter whose traffic
 * will silently never arrive. One filter per packet makes that a lookup.
 */
static void mqtt_pump_subscribes(struct inkwell_mqtt_client *proxy) {
    if (proxy->state != INKWELL_MQTT_CLIENT_READY || proxy->subscribe_id != 0U) {
        return;
    }
    ...
    uint8_t packet[INKWELL_MQTT_CLIENT_FILTER_MAX + 16U];
...
static bool mqtt_on_connack(struct inkwell_mqtt_client *proxy, const uint8_t *body, size_t len) {
    if (proxy->state != INKWELL_MQTT_CLIENT_GREETING) {
        mqtt_fail_own(proxy, INKWELL_MQTT_REFUSAL_PROTOCOL, 0U);
        return false;
    }
    uint8_t code = 0U;
    ...
    if (code != INKWELL_MQTT_CONNACK_ACCEPTED) {
         ...
         * password is a setting; a client that is not permitted is an account on
        ...
        case INKWELL_MQTT_CONNACK_BAD_CREDENTIALS:
            mqtt_fail_own(proxy, INKWELL_MQTT_REFUSAL_BAD_LOGIN, code);
            break;
        case INKWELL_MQTT_CONNACK_NOT_AUTHORISED:
            mqtt_fail_own(proxy, INKWELL_MQTT_REFUSAL_NOT_ALLOWED, code);
            break;
        case INKWELL_MQTT_CONNACK_UNAVAILABLE:
            mqtt_fail_own(proxy, INKWELL_MQTT_REFUSAL_BROKER_BUSY, code);
            break;
        default:
            mqtt_fail_own(proxy, INKWELL_MQTT_REFUSAL_OTHER, code);
            break;
        }
        return false;
    }
...
static bool mqtt_on_suback(struct inkwell_mqtt_client *proxy, const uint8_t *body, size_t len) {
    uint16_t id = 0U;
    uint8_t code = 0U;
    ...
    if (code == INKWELL_MQTT_SUBACK_FAILURE) {
         ...
         * is worth saying out loud, because the symptom is a filter whose traffic simply never
...
static bool mqtt_on_publish(struct inkwell_mqtt_client *proxy, uint8_t flags, const uint8_t *body,
                            size_t len) {
     ...
     * PUBACK ledger for messages whose value has expired by the time the caller hears them. The
     * message itself is still forwarded - it is real traffic and dropping it would be worse.
     */
    if (proxy->on_message == NULL) {
        return true;
    }
    /* A truncated topic is worse than none because the caller would route it incorrectly. */
    char topic[INKWELL_MQTT_CLIENT_TOPIC_MAX];
...
static bool mqtt_handle(struct inkwell_mqtt_client *proxy, const struct inkwell_mqtt_header *header,
                        const uint8_t *body) {
    switch (header->type) {
 ...
 */
static bool mqtt_consume(struct inkwell_mqtt_client *proxy) {
    size_t at = 0U;
 ...
 * grow.
 */
static void mqtt_read_ready(struct inkwell_mqtt_client *proxy) {
...
static void mqtt_send_connect(struct inkwell_mqtt_client *proxy) {
    ...
    params.keepalive_s = (uint16_t)INKWELL_MQTT_CLIENT_KEEPALIVE_S;
    /*
     ...
     * of stale traffic to replay after its value has passed.
     */
    params.clean_session = true;

    uint8_t packet[INKWELL_MQTT_CLIENT_CLIENT_ID_MAX + INKWELL_MQTT_CLIENT_USERNAME_MAX +
                   INKWELL_MQTT_CLIENT_PASSWORD_MAX + 32U];
    const int len = inkwell_mqtt_encode_connect(packet, sizeof packet, &params);
    if (len < 0) {
        mqtt_fail_own(proxy, INKWELL_MQTT_REFUSAL_BAD_ADDRESS, 0U);
        return;
    }
    ...
    proxy->last_heard_ms = proxy->now_ms;
    mqtt_set_state(proxy, INKWELL_MQTT_CLIENT_GREETING);
}

/* Drives the TLS handshake to completion, then hands over to the MQTT one. */
static void mqtt_secure(struct inkwell_mqtt_client *proxy) {
...
static int mqtt_fd_callback(int fd, uint32_t events, void *userdata) {
    ...
    if (proxy->state == INKWELL_MQTT_CLIENT_SECURING) {
...
static void mqtt_on_resolved(void *userdata, const struct inkwell_resolve_result *result) {
    ...
    if (proxy->state != INKWELL_MQTT_CLIENT_RESOLVING) {
        return;
    }
...
static void mqtt_attempt(struct inkwell_mqtt_client *proxy) {
    ...
    mqtt_set_state(proxy, INKWELL_MQTT_CLIENT_RESOLVING);
}
 ...
 * (mqtt_fail) or stopping it (inkwell_mqtt_client_stop), and the state it leaves behind is theirs
...
static int mqtt_target_split(const char *target, char *host, size_t host_len, uint16_t *port) {
...
int inkwell_mqtt_client_start(struct inkwell_mqtt_client *proxy,
     ...
     * "host", "host:port" and "[v6]:port" are accepted; a bare IPv6 literal has no port.
     */
    char host[INKWELL_MQTT_CLIENT_ADDRESS_MAX];
    uint16_t port = 0U;
    if (mqtt_target_split(config->address, host, sizeof host, &port) != 0) {
        return -EINVAL;
    }
     ...
     *
     * The parser reports an omitted port as zero, keeping an explicitly named port distinct.
     */
    if (port == 0U) {
        port = config->tls_enabled ? (uint16_t)INKWELL_MQTT_CLIENT_PORT_TLS
                                   : (uint16_t)INKWELL_MQTT_CLIENT_PORT;
    }
    proxy->port = port;
...
void inkwell_mqtt_client_stop(struct inkwell_mqtt_client *proxy) {
    if (proxy == NULL || proxy->state == INKWELL_MQTT_CLIENT_OFF) {
        return;
    }
    /*
     ...
     */
    if (proxy->state == INKWELL_MQTT_CLIENT_READY) {
        uint8_t packet[2];
    ...
    proxy->failures = 0U;
    proxy->retry_at_ms = 0U;
    mqtt_set_state(proxy, INKWELL_MQTT_CLIENT_OFF);
}
...
int inkwell_mqtt_client_publish(struct inkwell_mqtt_client *proxy, const char *topic,
    ...
    /* A wildcard is legal in a filter and forbidden in a published topic, and a broker's answer
       ...
       that flaps whenever one particular topic has traffic. */
    if (!inkwell_mqtt_topic_is_publishable(topic)) {
        proxy->stats.dropped++;
        return -EINVAL;
    }

    uint8_t packet[INKWELL_MQTT_CLIENT_PACKET_MAX];
    const int encoded =
...
void inkwell_mqtt_client_tick(struct inkwell_mqtt_client *proxy, uint64_t now_ms) {
    ...
    if (proxy->state == INKWELL_MQTT_CLIENT_OFF) {
        return;
    }
    if (proxy->state == INKWELL_MQTT_CLIENT_WAITING) {
     ...
     */
    if (proxy->more_to_read && (proxy->state == INKWELL_MQTT_CLIENT_GREETING ||
                                proxy->state == INKWELL_MQTT_CLIENT_READY)) {
        mqtt_read_ready(proxy);
    }
    ...
    /* The broker has said nothing for long enough that the connection is not there any more.
       This is what notices a network that went away without anybody closing anything. */
    if (proxy->last_heard_ms != 0U && now_ms > proxy->last_heard_ms &&
        now_ms - proxy->last_heard_ms > INKWELL_MQTT_CLIENT_SILENCE_TIMEOUT_MS) {
        mqtt_fail_net(proxy, INKWELL_NET_TIMED_OUT, 0);
        return;
    }
...
const char *inkwell_mqtt_client_failure_name(const struct inkwell_mqtt_client_failure *failure) {
    ...
    if (failure->refusal != INKWELL_MQTT_REFUSAL_NONE) {
...
inkwell_mqtt_client_failure(const struct inkwell_mqtt_client *proxy) {
    struct inkwell_mqtt_client_failure none;
