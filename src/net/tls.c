 ...
 * missing. A caller asks `inkwell_tls_available()` when it wants to say something useful about why,
 ...
 *
 * The same reasoning as a caller's own per-turn read budget, one level down and for the same
 * single event loop: such a bound counts calls into this file and cannot see a call that does
 * not return. A well-behaved server sends one ticket, so this is never reached in practice - it
 * is here so that a peer which streams them cannot hold the loop instead.
 */
#define INKWELL_TLS_TICKETS_PER_READ 8U

/*
 * Everything Mbed TLS needs for one session, on the heap.
 *
 * Heap rather than inline in `struct inkwell_tls_client` so that a caller's own header - and
 * therefore everything that includes it - does not have to include Mbed TLS's headers to know
 * how big a connection is. One allocation per connection attempt, not per message, which is the
 * same order as the socket it sits on.
 */
struct inkwell_tls_state {
 ...
 * on a device with little of it that file has to be /dev/urandom or the loop blocks.
 * third_party/mbedtls-config/inkwell_psa_crypto_config.h is where that is set, and why - the
 * measurements are in it, and they are worth reading before changing this.
 */
 ...
 *
 * The table itself is the caller's - see inkwell_tls_set_roots(). What is here is the parse of it,
 ...
 * header says so.
 */
static const struct inkwell_tls_ca_root *tls_ca_roots = NULL;
...
static bool tls_load_roots(struct inkwell_tls_client *tls) {
    if (tls_roots_ready) {
        return true;
    }
...
static void tls_record_error(struct inkwell_tls_client *tls, int code, const char *what) {
    char detail[128];
...
static int tls_bio_recv(void *ctx, unsigned char *buf, size_t len) {
    ...
    if (got == 0) {
        /* A socket EOF underneath a session that expected more is a truncated connection, not a
           ...
           itself. Telling them apart is what lets the caller distinguish "the peer hung up"
 ...
 */
static int tls_translate(struct inkwell_tls_client *tls, int code, const char *what) {
...
static void tls_release(struct inkwell_tls_client *tls) {
    struct inkwell_tls_state *state = tls->state;
    if (state == NULL) {
        return;
    }
...
int inkwell_tls_client_start(struct inkwell_tls_client *tls, inkwell_socket socket,
    ...
    if (ca_bundle == NULL || ca_bundle[0] == '\0') {
         ...
         * send whoever is debugging it after the server rather than the path.
         *
...
int inkwell_tls_client_handshake(struct inkwell_tls_client *tls) {
     ...
     * a person can act on: the server's certificate is not signed by anything in the bundle, or
...
int inkwell_tls_client_read(struct inkwell_tls_client *tls, uint8_t *out, size_t cap) {
     ...
     * the session behind the ticket - a server's first real message arrives in the same flight
     * as often as not - and the socket is empty by then, so epoll has nothing to report and the
     ...
     * within a turn or two - a well-behaved server sends one ticket. A peer that streams them
     * faster than they are decrypted would otherwise keep this function from returning, and
     * nothing above could stop it: a caller's own per-turn budget bounds calls *to* here, not
     * the work inside one. So a run this long gives the loop back with `more_to_read` set
     * instead, which is the same statement made one level up.
     *
    ...
    for (;;) {
        ...
        if (rc == MBEDTLS_ERR_SSL_RECEIVED_NEW_SESSION_TICKET) {
            if (++tickets < INKWELL_TLS_TICKETS_PER_READ) {
                continue;
            }
            tls->wants_write = false;
