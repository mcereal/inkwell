 ...
 * connect from a handheld over WiFi to a CDN takes well under a second.
 */
 ...
 * carrying no `User-Agent` at all - see inkwell_fetch_set_user_agent().
 */
...
struct inkwell_fetch_conn {
    ...
    char detail[INKWELL_FETCH_PATH_MAX + 96U];
 ...
 */
static void fetch_complete(struct inkwell_fetch *fetch, enum inkwell_fetch_outcome outcome) {
...
static bool fetch_sink(struct inkwell_fetch *fetch, const uint8_t *bytes, size_t len) {
    struct inkwell_fetch_conn *const conn = fetch->conn;
    if (conn->out_fd < 0) {
        if (!fetch_capture(conn, bytes, len)) {
            fetch_fail(fetch, INKWELL_FETCH_TOO_LARGE, "reply passed %zu bytes",
                       conn->response_max);
            return false;
        }
        return true;
...
static void fetch_finish(struct inkwell_fetch *fetch) {
    struct inkwell_fetch_conn *const conn = fetch->conn;
    if (conn->method == INKWELL_FETCH_HEAD &&
        !fetch_capture(conn, (const uint8_t *)conn->response.head, conn->response.head_len)) {
        fetch_fail(fetch, INKWELL_FETCH_TOO_LARGE, "head passed %zu bytes", conn->response_max);
        return;
    }
    fetch_complete(fetch, INKWELL_FETCH_OK);
}

/* ---- one hop ---------------------------------------------------------------------------- */

static void fetch_hop(struct inkwell_fetch *fetch);
 ...
 */
static bool fetch_on_head(struct inkwell_fetch *fetch) {
    struct inkwell_fetch_conn *const conn = fetch->conn;
    ...
    if (status == 301 || status == 302 || status == 303 || status == 307 || status == 308) {
        ...
        if (!inkwell_http_response_header(&conn->response, "location", &value, &len)) {
            fetch_fail(fetch, INKWELL_FETCH_HTTP_STATUS, "%d with no Location", status);
            return false;
        }
        if (conn->redirects >= INKWELL_FETCH_REDIRECTS_MAX) {
            fetch_fail(fetch, INKWELL_FETCH_PROTOCOL, "more than %u redirects",
                       INKWELL_FETCH_REDIRECTS_MAX);
            return false;
        }
        char location[INKWELL_HTTP_URL_MAX];
        struct inkwell_http_url next;
        if (len >= sizeof location) {
            fetch_fail(fetch, INKWELL_FETCH_PROTOCOL, "a %zu-byte Location", len);
            return false;
        }
        memcpy(location, value, len);
        location[len] = '\0';
        if (!inkwell_http_url_resolve(&conn->url, location, &next)) {
            fetch_fail(fetch, INKWELL_FETCH_PROTOCOL, "unusable Location %.96s", location);
            return false;
        }
        if (!next.tls) {
            fetch_fail(fetch, INKWELL_FETCH_PROTOCOL, "redirect off https to %.96s", next.host);
            return false;
        }
    ...
    if (status < 200 || status > 299) {
        fetch_fail(fetch, INKWELL_FETCH_HTTP_STATUS, "HTTP %d from %s", status, conn->url.host);
        return false;
    }
    if (conn->ranged && status != 206) {
        fetch_fail(fetch, INKWELL_FETCH_PROTOCOL, "range ignored: %d from %s", status,
                   conn->url.host);
        return false;
    }
...
static void fetch_on_close(struct inkwell_fetch *fetch, int rc) {
    ...
    if (conn->response.framing == INKWELL_HTTP_FRAMING_UNTIL_CLOSE && !clean) {
        fetch_fail(fetch, INKWELL_FETCH_NETWORK, "%s dropped an unframed body after %llu bytes",
                   conn->url.host, (unsigned long long)conn->response.body_received);
        return;
    }
    if (!inkwell_http_response_finish(&conn->response)) {
        fetch_fail(fetch, INKWELL_FETCH_NETWORK, "%s cut the body short after %llu bytes",
                   conn->url.host, (unsigned long long)conn->response.body_received);
        return;
    }
    fetch_finish(fetch);
...
static bool fetch_feed(struct inkwell_fetch *fetch, const uint8_t *in, size_t len) {
    struct inkwell_fetch_conn *const conn = fetch->conn;
    size_t at = 0U;
    while (at < len) {
        ...
        if (inkwell_http_response_failed(&conn->response)) {
            fetch_fail(fetch, INKWELL_FETCH_PROTOCOL, "bad reply from %s: %s", conn->url.host,
...
static void fetch_receive(struct inkwell_fetch *fetch) {
    struct inkwell_fetch_conn *const conn = fetch->conn;
    conn->more_to_read = false;
    for (unsigned i = 0U; i < FETCH_READS_PER_TURN; ++i) {
        const int rc = inkwell_tls_client_read(&conn->tls, conn->buffer, sizeof conn->buffer);
        if (rc == -EAGAIN) {
...
static void fetch_send(struct inkwell_fetch *fetch) {
    struct inkwell_fetch_conn *const conn = fetch->conn;
    while (conn->request_sent < conn->request_len) {
        const int rc = inkwell_tls_client_write(&conn->tls,
                                                (const uint8_t *)conn->request + conn->request_sent,
...
static void fetch_handshake(struct inkwell_fetch *fetch) {
    struct inkwell_fetch_conn *const conn = fetch->conn;
    const int rc = inkwell_tls_client_handshake(&conn->tls);
    if (rc == -EAGAIN) {
        fetch_arm(fetch, conn, conn->tls.wants_write);
        return;
    }
...
static void fetch_try_next(struct inkwell_fetch *fetch) {
    ...
    fetch_complete(fetch, INKWELL_FETCH_NETWORK);
}

/* The family that last connected goes first; the rest keep the resolver's order. */
static void fetch_prefer_family(struct inkwell_fetch *fetch, struct inkwell_fetch_conn *conn) {
...
/*
 * The addresses inkwell_fetch_connect_to() named, each a numeric address, comma-separated the way
 * curl's --resolve takes them. False when one does not parse.
 */
static bool fetch_connect_to_list(const struct inkwell_fetch *fetch,
...
bool inkwell_fetch_available(const struct inkwell_fetch *fetch) {
    return fetch != NULL && fetch->loop != NULL && inkwell_tls_available();
}
...
int inkwell_fetch_start(struct inkwell_fetch *fetch, const struct inkwell_fetch_request *request,
    ...
    bool agent = false;
    for (size_t i = 0U; i < INKWELL_FETCH_HEADERS_MAX && request->headers[i] != NULL; ++i) {
        if (!inkwell_str_copy(conn->headers[conn->header_count], INKWELL_FETCH_HEADER_MAX,
                              request->headers[i])) {
            free(conn);
            return -EINVAL;
        }
    ...
    if (!agent) {
        snprintf(conn->headers[conn->header_count++], INKWELL_FETCH_HEADER_MAX, "User-Agent: %s",
                 fetch_user_agent);
    }
