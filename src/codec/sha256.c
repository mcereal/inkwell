...
static void sha256_block(struct inkwell_sha256 *ctx, const uint8_t block[64]) {
    uint32_t w[64];
...
void inkwell_sha256_final(struct inkwell_sha256 *ctx, uint8_t out[INKWELL_SHA256_DIGEST_LEN]) {
    ...
    inkwell_sha256_update(ctx, &k_pad, 1U);
    static const uint8_t k_zero = 0x00U;
    while (ctx->block_len != 56U) {
        inkwell_sha256_update(ctx, &k_zero, 1U);
    }
...
void inkwell_sha256_hex(const uint8_t digest[INKWELL_SHA256_DIGEST_LEN], char *out,
                        size_t out_len) {
    if (out == NULL || out_len == 0U) {
        return;
    }
    out[0] = '\0';
    if (digest == NULL || out_len < INKWELL_SHA256_HEX_LEN) {
        return;
    }
    static const char k_hex[] = "0123456789abcdef";
    for (unsigned i = 0; i < INKWELL_SHA256_DIGEST_LEN; ++i) {
        out[2U * i] = k_hex[(digest[i] >> 4) & 0x0FU];
        out[2U * i + 1U] = k_hex[digest[i] & 0x0FU];
    }
    out[2U * INKWELL_SHA256_DIGEST_LEN] = '\0';
}
...
int inkwell_sha256_file(const char *path, uint8_t out[INKWELL_SHA256_DIGEST_LEN]) {
    ...
    for (;;) {
        ...
        if (read > 0U) {
            inkwell_sha256_update(&ctx, buffer, read);
        }
