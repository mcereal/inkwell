...
enum inkwell_zip_search inkwell_zip_find_member(const uint8_t *central, size_t len,
    ...
    for (uint32_t n = 0U; n < entries; ++n) {
        ...
        if ((size_t)name_len < INKWELL_ZIP_NAME_MAX) {
...
bool inkwell_zip_local_data_start(const uint8_t *header, size_t len,
    ...
    if (header == NULL || entry == NULL || out_offset == NULL ||
        len < INKWELL_ZIP_LOCAL_HEADER_SIZE) {
        return false;
    }
    ...
    *out_offset = entry->local_header_offset + INKWELL_ZIP_LOCAL_HEADER_SIZE + name_len + extra_len;
    return true;
}
