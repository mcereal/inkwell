...
static bool http_digit(char c) {
    return c >= '0' && c <= '9';
}

static char http_lower(char c) {
...
static bool http_url_byte_ok(unsigned char c) {
...
bool inkwell_http_url_parse(const char *url, struct inkwell_http_url *out) {
...
bool inkwell_http_url_resolve(const struct inkwell_http_url *base, const char *location,
    ...
    /* A scheme is ALPHA *(ALPHA / DIGIT / "+" / "-" / ".") ":" - and a reference that has one is
       absolute, whichever scheme it names. inkwell_http_url_parse() refuses the ones that are not
       HTTP. */
    size_t scheme = 0U;
    ...
    if (scheme > 0U && location[scheme] == ':') {
        return inkwell_http_url_parse(location, out);
    }

    if (location[0] == '/' && location[1] == '/') {
        char absolute[INKWELL_HTTP_URL_MAX + 8U];
        const int written =
            snprintf(absolute, sizeof absolute, "%s:%s", base->tls ? "https" : "http", location);
        if (written < 0 || (size_t)written >= sizeof absolute) {
            return false;
        }
        return inkwell_http_url_parse(absolute, out);
    }
    ...
    *out = *base;
    ...
    char merged[INKWELL_HTTP_URL_MAX];
    const int written =
...
static void http_fail(struct inkwell_http_response *response, enum inkwell_http_error error) {
 ...
 * that follows it.
 */
static bool http_parse_head(struct inkwell_http_response *response) {
    ...
    if (len < 12U || memcmp(line, "HTTP/1.", 7U) != 0 || !http_digit(line[7]) || line[8] != ' ' ||
        ...
        (len > 12U && line[12] != ' ')) {
        http_fail(response, INKWELL_HTTP_ERR_STATUS_LINE);
        return false;
    }
    ...
    if (status == 101) {
        http_fail(response, INKWELL_HTTP_ERR_UPGRADE);
        return false;
    }
    ...
    while (at < response->head_len) {
        ...
        if (!http_split_header(line, len, &name, &name_len, &value, &value_len)) {
            ...
            http_fail(response, INKWELL_HTTP_ERR_HEADER);
            return false;
        }

        if (http_equal_nocase(name, name_len, "content-length")) {
            if (value_len == 0U) {
                http_fail(response, INKWELL_HTTP_ERR_LENGTH);
                return false;
            }
            uint64_t parsed = 0U;
            for (size_t i = 0U; i < value_len; ++i) {
            ...
            /* Nothing but a single `chunked`. A compression coding would be a body this codec
            ...
            if (chunked || !http_equal_nocase(value, value_len, "chunked")) {
                http_fail(response, INKWELL_HTTP_ERR_ENCODING);
                return false;
            }
            chunked = true;
        }
...
static bool http_head_byte(struct inkwell_http_response *response, char c) {
    if (response->head_len + 1U >= sizeof response->head) {
        http_fail(response, INKWELL_HTTP_ERR_HEAD_TOO_LARGE);
        return true;
    }
...
static bool http_chunk_size_byte(struct inkwell_http_response *response, char c) {
    if (c != '\n') {
        if (response->chunk_line_len + 1U >= sizeof response->chunk_line) {
            http_fail(response, INKWELL_HTTP_ERR_CHUNK);
            return false;
        }
...
void inkwell_http_response_init(struct inkwell_http_response *response, bool head_request) {
    if (response == NULL) {
        return;
    }
...
size_t inkwell_http_response_feed(struct inkwell_http_response *response, const uint8_t *in,
                                  size_t len, const uint8_t **body, size_t *body_len) {
    if (body != NULL) {
        *body = NULL;
    }
    if (body_len != NULL) {
    ...
    while (at < len) {
        ...
        case HTTP_PHASE_TRAILER: {
            ...
            at++;
            if (++response->trailer_len >= INKWELL_HTTP_HEAD_MAX) {
                http_fail(response, INKWELL_HTTP_ERR_HEAD_TOO_LARGE);
                return at;
            }
            if (c != '\n') {
...
bool inkwell_http_response_finish(struct inkwell_http_response *response) {
    if (response == NULL) {
        return false;
    }
    ...
    if (response->phase != HTTP_PHASE_FAILED) {
        http_fail(response, INKWELL_HTTP_ERR_TRUNCATED);
    }
    return false;
}

bool inkwell_http_response_head_done(const struct inkwell_http_response *response) {
    return response != NULL && response->status != 0;
}

bool inkwell_http_response_done(const struct inkwell_http_response *response) {
    return response != NULL && response->phase == HTTP_PHASE_DONE;
}

bool inkwell_http_response_failed(const struct inkwell_http_response *response) {
    return response != NULL && response->phase == HTTP_PHASE_FAILED;
}

bool inkwell_http_response_header(const struct inkwell_http_response *response, const char *name,
                                  const char **value, size_t *value_len) {
    if (!inkwell_http_response_head_done(response) || name == NULL || value == NULL ||
        value_len == NULL) {
        return false;
    }
    const char *line = NULL;
...
const char *inkwell_http_error_name(enum inkwell_http_error error) {
    static const char *const k_names[INKWELL_HTTP_ERROR_COUNT] = {
        [INKWELL_HTTP_OK] = "ok",
        [INKWELL_HTTP_ERR_STATUS_LINE] = "status line",
        [INKWELL_HTTP_ERR_HEADER] = "header",
        [INKWELL_HTTP_ERR_HEAD_TOO_LARGE] = "head too large",
        [INKWELL_HTTP_ERR_LENGTH] = "content length",
        [INKWELL_HTTP_ERR_ENCODING] = "transfer encoding",
        [INKWELL_HTTP_ERR_CHUNK] = "chunk",
        [INKWELL_HTTP_ERR_UPGRADE] = "upgrade",
        [INKWELL_HTTP_ERR_TRUNCATED] = "truncated",
    };
    if ((unsigned)error >= (unsigned)INKWELL_HTTP_ERROR_COUNT || k_names[error] == NULL) {
        return "unknown";
    }
