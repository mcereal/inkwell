...
static void json_skip_space(struct inkwell_json *json) {
...
static char json_peek(struct inkwell_json *json) {
    json_skip_space(json);
    return json->cursor < json->end ? *json->cursor : '\0';
}

static bool json_take(struct inkwell_json *json, char expected) {
 ...
 */
static bool json_skip_string_body(struct inkwell_json *json) {
    while (json->cursor < json->end) {
        const char c = *json->cursor++;
        if (c == '"') {
            return true;
 ...
 */
static bool json_read_string_body(struct inkwell_json *json, char *out, size_t out_len) {
    size_t written = 0U;
...
bool inkwell_json_skip_value(struct inkwell_json *json) {
    ...
    while (json->cursor < json->end) {
        ...
        if (c == '{' || c == '[') {
            depth++;
            if (depth > INKWELL_JSON_MAX_DEPTH) {
                return false;
            }
            continue;
        }
...
bool inkwell_json_object_find(struct inkwell_json *json, const char *key) {
    if (json == NULL || key == NULL || !inkwell_json_enter_object(json)) {
        return false;
    }
    char name[64];
    while (inkwell_json_next_key(json, name, sizeof name)) {
        if (strcmp(name, key) == 0) {
            return true;
        }
        if (!inkwell_json_skip_value(json)) {
            return false;
        }
    }
    return false;
