 ...
 * than dropping the connection, which is the one refusal a caller can explain properly.
 */
...
static int mqtt_read_varint(const uint8_t *data, size_t len, size_t *out) {
    ...
    for (size_t i = 0U; i < INKWELL_MQTT_REMAINING_BYTES_MAX; ++i) {
        if (i >= len) {
            return 0;
        }
...
static size_t mqtt_optional_len(const char *text) {
 ...
 */
static bool mqtt_flags_are_legal(enum inkwell_mqtt_packet_type type, uint8_t flags) {
    switch (type) {
    case INKWELL_MQTT_PUBLISH:
        /* Free: DUP, QoS and RETAIN all live here. A QoS of 3 is malformed, but that is
           inkwell_mqtt_decode_publish()'s to refuse - a packet this decoder cannot interpret can
...
int inkwell_mqtt_decode_header(const uint8_t *data, size_t len, struct inkwell_mqtt_header *out) {
    ...
    const enum inkwell_mqtt_packet_type type = (enum inkwell_mqtt_packet_type)raw_type;
...
int inkwell_mqtt_decode_suback(const uint8_t *body, size_t len, uint16_t *packet_id,
                               uint8_t *code) {
 ...
 */
static uint8_t *mqtt_begin_packet(uint8_t *out, size_t cap, enum inkwell_mqtt_packet_type type,
                                  uint8_t flags, size_t body_len, size_t *total) {
    if (body_len > INKWELL_MQTT_REMAINING_MAX) {
        return NULL;
    }
...
int inkwell_mqtt_encode_connect(uint8_t *out, size_t cap,
    ...
    size_t total = 0U;
    uint8_t *body = mqtt_begin_packet(out, cap, INKWELL_MQTT_CONNECT, 0U, body_len, &total);
    if (body == NULL) {
        return -ENOSPC;
    }
...
int inkwell_mqtt_encode_publish(uint8_t *out, size_t cap, const char *topic, const uint8_t *payload,
    ...
    uint8_t *body = mqtt_begin_packet(out, cap, INKWELL_MQTT_PUBLISH, flags, body_len, &total);
    if (body == NULL) {
        return -ENOSPC;
    }
...
int inkwell_mqtt_encode_subscribe(uint8_t *out, size_t cap, uint16_t packet_id,
                                  const char *filter) {
    ...
    size_t total = 0U;
    uint8_t *body =
        mqtt_begin_packet(out, cap, INKWELL_MQTT_SUBSCRIBE, MQTT_FLAGS_QOS1, body_len, &total);
    if (body == NULL) {
        return -ENOSPC;
    }
...
int inkwell_mqtt_encode_empty(uint8_t *out, size_t cap, enum inkwell_mqtt_packet_type type) {
    ...
    if (type != INKWELL_MQTT_PINGREQ && type != INKWELL_MQTT_DISCONNECT) {
        return -EINVAL;
    }
    size_t total = 0U;
