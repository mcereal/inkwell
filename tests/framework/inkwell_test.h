#ifndef INKWELL_TEST_H
#define INKWELL_TEST_H
...
struct inkwell_test_case {
    const char *name;
    const char *category;
    const char *file;
    int line;
    void (*fn)(void);
    struct inkwell_test_case *next;
};
 ...
 *
 *     INKWELL_TEST_CASE(config_defaults, unit) {
 *         struct mesh_app_config config = mesh_app_config_default();
 *         INKWELL_TEST_FAIL_IF(config.idle_timeout_ms != 1000, "idle timeout should default to
 * 1s"); record_success(test_name);
 *     }
 */
#define INKWELL_TEST_CASE(case_name, case_category)                                                \
    static void inkwell_test_body_##case_name(const char *test_name);                              \
    static void inkwell_test_entry_##case_name(void) {                                             \
        inkwell_test_body_##case_name(#case_name);                                                 \
    }                                                                                              \
    static void inkwell_test_ctor_##case_name(void) __attribute__((constructor));                  \
    static void inkwell_test_ctor_##case_name(void) {                                              \
        static struct inkwell_test_case node;                                                      \
        inkwell_test_register(&node, #case_name, #case_category, __FILE__, __LINE__,               \
                              inkwell_test_entry_##case_name);                                     \
    }                                                                                              \
    static void inkwell_test_body_##case_name(const char *test_name)

/* Records `message` against the running case and returns from it when `condition` holds. */
#define INKWELL_TEST_FAIL_IF(condition, message)                                                   \
    do {                                                                                           \
        if (condition) {                                                                           \
