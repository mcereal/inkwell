 ...
 *
 * The clock is synthetic throughout. `inkwell_mqtt_client_tick()` takes the time rather than
 * reading it, so a case can step past a five-second backoff or a ninety-second silence timeout
 * without waiting for either - which is the only reason those paths are testable at all.
 */
...
struct fake_broker {
     ...
     * sets this; see mqtt_client_reads_past_a_session_ticket().
     */
    uint16_t tickets;
...
static bool broker_tls_start(struct fake_broker *broker) {
     ...
     * is what this client did against a public broker, and what a handshake-only fixture
...
struct proxy_probe {
    ...
    unsigned messages;
    char last_topic[INKWELL_MQTT_CLIENT_TOPIC_MAX];
...
static void probe_turn(struct proxy_probe *probe, uint64_t step_ms) {
    ...
    inkwell_mqtt_client_tick(&probe->proxy, probe->now_ms);
}

static bool probe_until_state(struct proxy_probe *probe, enum inkwell_mqtt_client_state state,
                              unsigned turns) {
    for (unsigned turn = 0U; turn < turns; ++turn) {
        if (inkwell_mqtt_client_state(&probe->proxy) == state) {
            return true;
        }
        probe_turn(probe, 10U);
    }
    return inkwell_mqtt_client_state(&probe->proxy) == state;
}
...
static void probe_config(struct inkwell_mqtt_client_config *config, uint16_t port) {
    memset(config, 0, sizeof *config);
    snprintf(config->address, sizeof config->address, "127.0.0.1:%u", (unsigned)port);
    snprintf(config->client_id, sizeof config->client_id, "inkwell-test");
}
...
static bool probe_start(struct proxy_probe *probe) {
    ...
    return inkwell_mqtt_client_init(&probe->proxy, &probe->loop) == 0;
}

static void probe_stop(struct proxy_probe *probe) {
    inkwell_mqtt_client_shutdown(&probe->proxy);
...
static bool probe_connect(struct proxy_probe *probe) {
    ...
    if (inkwell_mqtt_client_start(&probe->proxy, &config, probe_on_message, NULL, probe,
                                  probe->now_ms) != 0) {
        return false;
    }
    ...
    return probe_until_state(probe, INKWELL_MQTT_CLIENT_READY, 100U);
}

/* ------------------------------------------------------------------ getting connected */

INKWELL_TEST_CASE(mqtt_client_names_each_failure_for_logs, unit) {
    struct inkwell_mqtt_client_failure failure;
    for (int refusal = 1; refusal < (int)INKWELL_MQTT_REFUSAL_COUNT; ++refusal) {
        memset(&failure, 0, sizeof failure);
        failure.refusal = (enum inkwell_mqtt_refusal)refusal;
        const char *const name = inkwell_mqtt_client_failure_name(&failure);
        if (strcmp(name, "ok") == 0 || strcmp(name, "none") == 0) {
            record_failure(test_name, "an MQTT refusal must not be named as success");
            return;
        }
    }
    for (int reason = 1; reason < (int)INKWELL_NET_REASON_COUNT; ++reason) {
        memset(&failure, 0, sizeof failure);
        failure.net.reason = (enum inkwell_net_reason)reason;
        const char *const name = inkwell_mqtt_client_failure_name(&failure);
        if (strcmp(name, "ok") == 0 || strcmp(name, "none") == 0) {
            record_failure(test_name, "a network failure must not be named as success");
            return;
        }
    }
    memset(&failure, 0, sizeof failure);
    if (strcmp(inkwell_mqtt_client_failure_name(&failure), "none") != 0) {
        record_failure(test_name, "an empty failure should be named none");
        return;
    }
    record_success(test_name);
}

INKWELL_TEST_CASE(mqtt_client_greets_a_broker, unit) {
    ...
    struct inkwell_mqtt_client_config config;
    probe_config(&config, probe.broker.port);
    snprintf(config.username, sizeof config.username, "visitor");
    ...
    if (!holds(body, header.remaining, "inkwell-test") ||
        !holds(body, header.remaining, "visitor") || !holds(body, header.remaining, "swordfish")) {
    ...
    if (inkwell_mqtt_client_is_ready(&probe.proxy)) {
    ...
    if (inkwell_mqtt_client_stats(&probe.proxy).connections != 1U) {
 ...
 */
INKWELL_TEST_CASE(mqtt_client_reports_a_refused_login, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    if (!probe_until_state(&probe, INKWELL_MQTT_CLIENT_WAITING, 100U)) {
        record_failure(test_name, "a refused login should end the attempt");
        goto cleanup;
    }
    const struct inkwell_mqtt_client_failure failure = inkwell_mqtt_client_failure(&probe.proxy);
    if (failure.refusal != INKWELL_MQTT_REFUSAL_BAD_LOGIN) {
        record_failure(test_name, "a rejected password is its own refusal, not a generic one");
        goto cleanup;
    }
    if (inkwell_mqtt_client_stats(&probe.proxy).connections != 0U) {
    ...
    probe.now_ms += INKWELL_MQTT_CLIENT_BACKOFF_BASE_MS / 2U;
    inkwell_mqtt_client_tick(&probe.proxy, probe.now_ms);
    if (inkwell_mqtt_client_state(&probe.proxy) != INKWELL_MQTT_CLIENT_WAITING) {
...
INKWELL_TEST_CASE(mqtt_client_refuses_what_it_cannot_connect_to, unit) {
    struct inkwell_mqtt_client proxy;
    struct inkwell_mqtt_client_config config;
    probe_config(&config, 1883U);

    /* No loop: nothing to watch a socket with, so nothing is opened. */
    (void)inkwell_mqtt_client_init(&proxy, NULL);
    if (inkwell_mqtt_client_start(&proxy, &config, NULL, NULL, NULL, 0U) != -ENOTSUP) {
    ...
    (void)inkwell_mqtt_client_init(&proxy, &loop);
    ...
    struct inkwell_mqtt_client_config broken = config;
    snprintf(broken.address, sizeof broken.address, "[not-an-address");
    if (inkwell_mqtt_client_start(&proxy, &broken, NULL, NULL, NULL, 0U) != -EINVAL) {
    ...
    if (inkwell_mqtt_client_state(&proxy) != INKWELL_MQTT_CLIENT_OFF) {
...
INKWELL_TEST_CASE(mqtt_client_subscribes_one_filter_at_a_time, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    }
    if (inkwell_mqtt_client_subscribe(&probe.proxy, "example/first/#") != 0 ||
        inkwell_mqtt_client_subscribe(&probe.proxy, "example/second/#") != 0) {
        record_failure(test_name, "filters added before a connection should be held");
        goto cleanup;
    }
    /* Adding the same one twice is not a failure: a caller may re-derive the whole set whenever
       its policy changes, and having to diff it first would be the caller's bug. */
    if (inkwell_mqtt_client_subscribe(&probe.proxy, "example/first/#") != -EEXIST) {
    ...
    if (!holds(body, header.remaining, "first")) {
    ...
    if (second.type != INKWELL_MQTT_SUBSCRIBE || !holds(second_body, second.remaining, "second")) {
 ...
 * with topic ACLs permitting some filters and not others is an ordinary configuration rather
 * than a broken one.
 */
INKWELL_TEST_CASE(mqtt_client_survives_a_refused_filter, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    }
    if (inkwell_mqtt_client_subscribe(&probe.proxy, "example/forbidden/#") != 0 ||
        inkwell_mqtt_client_subscribe(&probe.proxy, "example/allowed/#") != 0 ||
        !probe_connect(&probe)) {
...
INKWELL_TEST_CASE(mqtt_client_publishes_what_the_caller_gives_it, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    /* Before a connection, a publish is refused and counted. This is the ordinary state of the
       thing while a link is down, and the caller may keep offering - so it is a return code,
       not a log line. */
    static const uint8_t payload[] = {0x01U, 0x02U, 0x03U, 0x04U};
    if (inkwell_mqtt_client_publish(&probe.proxy, "example/first/!a", payload, sizeof payload,
                                    false) != -ENOTCONN) {
        record_failure(test_name, "a publish with no broker should be refused");
        goto cleanup;
    }
    if (inkwell_mqtt_client_stats(&probe.proxy).dropped != 1U) {
    ...
    static const char k_topic[] = "example/first/!a";
    if (message.topic_len != sizeof k_topic - 1U ||
        memcmp(message.topic, k_topic, sizeof k_topic - 1U) != 0 ||
     ...
     * flaps whenever one particular topic had traffic - so it is refused here instead.
     */
    if (inkwell_mqtt_client_publish(&probe.proxy, "example/#", payload, sizeof payload, false) !=
        -EINVAL) {
...
INKWELL_TEST_CASE(mqtt_client_delivers_what_the_broker_sends, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    const int len = inkwell_mqtt_encode_publish(packet, sizeof packet, "example/first/!b", payload,
    ...
    if (strcmp(probe.last_topic, "example/first/!b") != 0 ||
 ...
 * message larger than this bounded client accepts; the bytes still have to be consumed in
 ...
 */
INKWELL_TEST_CASE(mqtt_client_skips_an_oversized_message, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    /* Comfortably past INKWELL_MQTT_CLIENT_PACKET_MAX, and past a single read besides, so the skip
       has to survive being spread over several of them. */
    static uint8_t big[6000];
    memset(big, 0x5AU, sizeof big);
    uint8_t oversized[sizeof big + 64U];
    const int big_len = inkwell_mqtt_encode_publish(oversized, sizeof oversized, "example/big", big,
                                                    sizeof big, false);
    uint8_t small[128];
    static const uint8_t payload[] = {0x11U, 0x22U};
    const int small_len = inkwell_mqtt_encode_publish(small, sizeof small, "example/small", payload,
                                                      sizeof payload, false);
    ...
    if (strcmp(probe.last_topic, "example/small") != 0 ||
        probe.last_payload_len != sizeof payload) {
        record_failure(test_name, "the message after the skipped one should be intact");
        goto cleanup;
    }
    if (inkwell_mqtt_client_stats(&probe.proxy).skipped == 0U) {
...
INKWELL_TEST_CASE(mqtt_client_pings_a_quiet_broker, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    probe.now_ms += INKWELL_MQTT_CLIENT_PING_INTERVAL_MS + 1U;
    struct inkwell_mqtt_header header;
    const uint8_t *body = NULL;
 ...
 *
 * This is the ordinary case on a mobile device carried out of WiFi range: it leaves a socket
 * that is open, writable, and connected to nobody. Nothing but the clock notices.
 */
INKWELL_TEST_CASE(mqtt_client_gives_up_on_a_silent_broker, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    probe.now_ms += INKWELL_MQTT_CLIENT_SILENCE_TIMEOUT_MS + 1U;
    inkwell_mqtt_client_tick(&probe.proxy, probe.now_ms);
    if (inkwell_mqtt_client_state(&probe.proxy) != INKWELL_MQTT_CLIENT_WAITING) {
        record_failure(test_name, "a silent broker should end the connection");
        goto cleanup;
    }
    if (inkwell_mqtt_client_failure(&probe.proxy).net.reason != INKWELL_NET_TIMED_OUT) {
...
INKWELL_TEST_CASE(mqtt_client_reconnects_after_a_hangup, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    probe.now_ms += INKWELL_MQTT_CLIENT_BACKOFF_BASE_MS + 1U;
    inkwell_mqtt_client_tick(&probe.proxy, probe.now_ms);
    ...
    if (inkwell_mqtt_client_stats(&probe.proxy).connections != 2U) {
 ...
 */
INKWELL_TEST_CASE(mqtt_client_says_goodbye, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    inkwell_mqtt_client_stop(&probe.proxy);
    if (inkwell_mqtt_client_state(&probe.proxy) != INKWELL_MQTT_CLIENT_OFF) {
    ...
    probe.now_ms += INKWELL_MQTT_CLIENT_BACKOFF_MAX_MS;
    inkwell_mqtt_client_tick(&probe.proxy, probe.now_ms);
    if (inkwell_mqtt_client_state(&probe.proxy) != INKWELL_MQTT_CLIENT_OFF) {
 ...
 */
INKWELL_TEST_CASE(mqtt_client_drops_a_desynchronised_stream, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
 ...
 */
INKWELL_TEST_CASE(mqtt_client_connects_over_tls, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    char bundle[128];
    snprintf(bundle, sizeof bundle, "/tmp/inkwell-test-ca-%u.pem", (unsigned)getpid());
    if (!write_pem(bundle, broker_cert_pem)) {
        record_failure(test_name, "could not write the CA bundle");
        goto cleanup;
    }
    inkwell_mqtt_client_set_ca_bundle(&probe.proxy, bundle);
    ...
    broker_connack(&probe.broker, INKWELL_MQTT_CONNACK_ACCEPTED);
    if (!probe_until_state(&probe, INKWELL_MQTT_CLIENT_READY, 400U)) {
    ...
    if (inkwell_mqtt_client_publish(&probe.proxy, "example/tls/!c", payload, sizeof payload,
                                    false) != 0) {
    ...
    const int len = inkwell_mqtt_encode_publish(inbound, sizeof inbound, "example/tls/!d", reply,
                                                sizeof reply, false);
    if (len < 0) {
        record_failure(test_name, "the fixture could not build a PUBLISH");
        goto cleanup;
 ...
 * single turn.
 */
INKWELL_TEST_CASE(mqtt_client_reads_past_a_session_ticket, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    char bundle[128];
    snprintf(bundle, sizeof bundle, "/tmp/inkwell-test-ticket-ca-%u.pem", (unsigned)getpid());
    if (!write_pem(bundle, broker_cert_pem)) {
        record_failure(test_name, "could not write the CA bundle");
        goto cleanup;
    }
    ...
    const int len = inkwell_mqtt_encode_publish(inbound, sizeof inbound, "example/ticket/!e", reply,
                                                sizeof reply, false);
    if (len < 0) {
        record_failure(test_name, "the fixture could not build a PUBLISH");
        goto cleanup;
 ...
 *
 * Twelve rather than the three above, because the TLS read budget caps how many one read may
 * consume before handing back -EAGAIN with `more_to_read` set, so
 ...
 */
INKWELL_TEST_CASE(mqtt_client_survives_a_run_of_tickets, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    char bundle[128];
    snprintf(bundle, sizeof bundle, "/tmp/inkwell-test-run-ca-%u.pem", (unsigned)getpid());
    if (!write_pem(bundle, broker_cert_pem)) {
        record_failure(test_name, "could not write the CA bundle");
        goto cleanup;
    }
    ...
    const int len = inkwell_mqtt_encode_publish(inbound, sizeof inbound, "example/run/!f", reply,
                                                sizeof reply, false);
    if (len < 0) {
        record_failure(test_name, "the fixture could not build a PUBLISH");
        goto cleanup;
 ...
 */
INKWELL_TEST_CASE(mqtt_client_refuses_an_unknown_certificate, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    char bundle[128];
    snprintf(bundle, sizeof bundle, "/tmp/inkwell-test-stranger-%u.pem", (unsigned)getpid());
    ...
    if (!probe_until_state(&probe, INKWELL_MQTT_CLIENT_WAITING, 400U)) {
    ...
    if (inkwell_mqtt_client_failure(&probe.proxy).net.reason != INKWELL_NET_TLS) {
    ...
    if (inkwell_mqtt_client_tls_error(&probe.proxy)[0] == '\0') {
 ...
 */
INKWELL_TEST_CASE(mqtt_client_names_a_missing_bundle, unit) {
    struct proxy_probe probe;
    if (!probe_start(&probe)) {
        record_failure(test_name, "the harness did not start");
        return;
    ...
    probe.broker.tls = true;
    static const char k_missing[] = "/nonexistent/inkwell/ca.pem";
    inkwell_mqtt_client_set_ca_bundle(&probe.proxy, k_missing);
    ...
    if (strstr(inkwell_mqtt_client_tls_error(&probe.proxy), k_missing) == NULL) {
 ...
 * blocks until the kernel's entropy *estimate* recovers, and small devices may refill it at a
 * crawl. The seeding happens inline on the one event-loop thread, so every client operation
 * stops with it.
 *
 ...
 */
INKWELL_TEST_CASE(mqtt_client_never_seeds_from_the_blocking_random_device, unit) {
