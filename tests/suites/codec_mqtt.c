...
INKWELL_TEST_CASE(mqtt_connect_is_the_3_1_1_shape, unit) {
    ...
    uint8_t out[64];
    const int written = inkwell_mqtt_encode_connect(out, sizeof out, &params);
...
INKWELL_TEST_CASE(mqtt_connect_carries_credentials, unit) {
    struct inkwell_mqtt_connect params;
    memset(&params, 0, sizeof params);
    params.client_id = "b";
    params.username = "u";
 ...
 */
INKWELL_TEST_CASE(mqtt_connect_refuses_what_a_broker_would, unit) {
    uint8_t out[64];

    struct inkwell_mqtt_connect orphan_password;
...
INKWELL_TEST_CASE(mqtt_publish_is_the_3_1_1_shape, unit) {
    static const uint8_t payload[] = {0xDEU, 0xADU};
    uint8_t out[32];
    const int written =
        inkwell_mqtt_encode_publish(out, sizeof out, "msh/2/e", payload, sizeof payload, false);
    ...
    const int retained =
        inkwell_mqtt_encode_publish(out, sizeof out, "msh/2/e", payload, sizeof payload, true);
 ...
 * fails to parse.
 */
INKWELL_TEST_CASE(mqtt_publish_finds_the_payload_at_every_qos, unit) {
    ...
    if (inkwell_mqtt_decode_publish(0x00U, qos0, sizeof qos0, &message) != 0 ||
    ...
    if (inkwell_mqtt_decode_publish(0x02U, qos1, sizeof qos1, &message) != 0 ||
        !bytes_match(message.payload, message.payload_len, want_payload, sizeof want_payload) ||
    ...
    if (inkwell_mqtt_decode_publish(0x09U, qos0, sizeof qos0, &message) != 0 || !message.retained ||
        !message.duplicate ||
        !bytes_match(message.payload, message.payload_len, want_payload, sizeof want_payload)) {
    ...
    if (inkwell_mqtt_decode_publish(0x06U, qos1, sizeof qos1, &message) != -EBADMSG) {
...
INKWELL_TEST_CASE(mqtt_publish_refuses_a_truncated_body, unit) {
    struct inkwell_mqtt_incoming message;

    /* A topic length that runs past the end of the body it was read from. */
    static const uint8_t overrun[] = {0x00U, 0x40U, 'a'};
    if (inkwell_mqtt_decode_publish(0x00U, overrun, sizeof overrun, &message) != -EBADMSG) {
    ...
    if (inkwell_mqtt_decode_publish(0x00U, stub, sizeof stub, &message) != -EBADMSG) {
    ...
    if (inkwell_mqtt_decode_publish(0x02U, no_id, sizeof no_id, &message) != -EBADMSG) {
    ...
    if (inkwell_mqtt_decode_publish(0x00U, no_topic, sizeof no_topic, &message) != -EBADMSG) {
    ...
    if (inkwell_mqtt_decode_publish(0x00U, empty, sizeof empty, &message) != 0 ||
...
INKWELL_TEST_CASE(mqtt_subscribe_is_flagged_qos1, unit) {
    uint8_t out[32];
    const int written = inkwell_mqtt_encode_subscribe(out, sizeof out, 0x0001U, "msh/2/e/#");
    ...
    if (inkwell_mqtt_encode_subscribe(out, sizeof out, 0U, "msh/#") != -EINVAL) {
...
INKWELL_TEST_CASE(mqtt_header_waits_for_every_byte, unit) {
    ...
    for (size_t len = 0U; len < 2U; ++len) {
        if (inkwell_mqtt_decode_header(stream, len, &header) != 0) {
            record_failure(test_name, "a partial header should report 'not yet', not an error");
            return;
        }
    }
    if (inkwell_mqtt_decode_header(stream, 2U, &header) != 2 ||
        header.type != INKWELL_MQTT_PINGREQ || header.remaining != 0U) {
        record_failure(test_name, "a PINGREQ is two bytes and no body");
        return;
    }
    if (inkwell_mqtt_decode_header(stream + 2, sizeof stream - 2U, &header) != 2 ||
        header.type != INKWELL_MQTT_CONNACK || header.remaining != 2U || header.header_len != 2U) {
 ...
 */
INKWELL_TEST_CASE(mqtt_header_reads_every_length_width, unit) {
    static const struct {
        uint8_t bytes[5];
        size_t len;
        size_t remaining;
        ...
        {{0x30U, 0xFFU, 0xFFU, 0xFFU, 0x7FU}, 5U, INKWELL_MQTT_REMAINING_MAX, 5U},
    };

    struct inkwell_mqtt_header header;
    for (size_t i = 0U; i < sizeof cases / sizeof cases[0]; ++i) {
        const int result = inkwell_mqtt_decode_header(cases[i].bytes, cases[i].len, &header);
    ...
    if (inkwell_mqtt_decode_header(overlong, sizeof overlong, &header) != -EBADMSG) {
 ...
 * connection instead.
 */
INKWELL_TEST_CASE(mqtt_header_refuses_a_desynchronised_stream, unit) {
    struct inkwell_mqtt_header header;
    static const struct {
        uint8_t bytes[2];
        const char *why;
    ...
    for (size_t i = 0U; i < sizeof refused / sizeof refused[0]; ++i) {
        if (inkwell_mqtt_decode_header(refused[i].bytes, 2U, &header) != -EBADMSG) {
    ...
    if (inkwell_mqtt_decode_header(publish, sizeof publish, &header) != 2 ||
        header.type != INKWELL_MQTT_PUBLISH || header.flags != 0x0BU) {
...
INKWELL_TEST_CASE(mqtt_acknowledgements_decode, unit) {
    uint8_t code = 0xFFU;
    bool session_present = true;

    static const uint8_t accepted[] = {0x00U, 0x00U};
    if (inkwell_mqtt_decode_connack(accepted, sizeof accepted, &code, &session_present) != 0 ||
        code != INKWELL_MQTT_CONNACK_ACCEPTED || session_present) {
        record_failure(test_name, "an accepted CONNACK should decode");
        return;
    }
    static const uint8_t refused[] = {0x01U, 0x05U};
    if (inkwell_mqtt_decode_connack(refused, sizeof refused, &code, &session_present) != 0 ||
        code != INKWELL_MQTT_CONNACK_NOT_AUTHORISED || !session_present) {
    ...
    if (inkwell_mqtt_decode_connack(reserved_set, sizeof reserved_set, &code, NULL) != -EBADMSG ||
        inkwell_mqtt_decode_connack(accepted, 3U, &code, NULL) != -EBADMSG) {
    ...
    if (inkwell_mqtt_decode_suback(granted, sizeof granted, &packet_id, &code) != 0 ||
    ...
    if (inkwell_mqtt_decode_suback(declined, sizeof declined, &packet_id, &code) != 0 ||
        code != INKWELL_MQTT_SUBACK_FAILURE) {
 ...
 */
INKWELL_TEST_CASE(mqtt_encoders_refuse_a_buffer_one_byte_short, unit) {
    uint8_t out[64];
    struct inkwell_mqtt_connect params;
    memset(&params, 0, sizeof params);
    params.client_id = "brick";
    ...
    const int connect_len = inkwell_mqtt_encode_connect(out, sizeof out, &params);
    const int publish_len = inkwell_mqtt_encode_publish(out, sizeof out, "a", NULL, 0U, false);
    const int subscribe_len = inkwell_mqtt_encode_subscribe(out, sizeof out, 1U, "a");
    ...
    if (inkwell_mqtt_encode_connect(out, (size_t)connect_len - 1U, &params) != -ENOSPC ||
        inkwell_mqtt_encode_publish(out, (size_t)publish_len - 1U, "a", NULL, 0U, false) !=
            -ENOSPC ||
        inkwell_mqtt_encode_subscribe(out, (size_t)subscribe_len - 1U, 1U, "a") != -ENOSPC ||
        inkwell_mqtt_encode_empty(out, 1U, INKWELL_MQTT_PINGREQ) != -ENOSPC) {
 ...
 */
INKWELL_TEST_CASE(mqtt_publish_grows_its_length_prefix, unit) {
    uint8_t payload[126];
    memset(payload, 0x5AU, sizeof payload);
    uint8_t out[160];

    /* topic "a" is 3 bytes on the wire, so 125 bytes of payload is a body of exactly 128. */
    const int written = inkwell_mqtt_encode_publish(out, sizeof out, "a", payload, 125U, false);
    if (written != 1 + 2 + 128) {
        record_failure(test_name, "a 128-byte body needs a two-byte length");
        return;
    }
    struct inkwell_mqtt_header header;
    if (inkwell_mqtt_decode_header(out, (size_t)written, &header) != 3 ||
        header.remaining != 128U) {
        record_failure(test_name, "the two-byte length should read back");
        return;
    }

    /* And one less stays inside a single byte. */
    const int smaller = inkwell_mqtt_encode_publish(out, sizeof out, "a", payload, 124U, false);
    if (smaller != 1 + 1 + 127 || inkwell_mqtt_decode_header(out, (size_t)smaller, &header) != 2 ||
...
INKWELL_TEST_CASE(mqtt_topics_reject_wildcards, unit) {
    if (!inkwell_mqtt_topic_is_publishable("msh/2/e/LongFast/!abcd1234")) {
    ...
    if (inkwell_mqtt_topic_is_publishable("msh/2/e/#") ||
        inkwell_mqtt_topic_is_publishable("msh/+/e") || inkwell_mqtt_topic_is_publishable("") ||
        inkwell_mqtt_topic_is_publishable(NULL)) {
