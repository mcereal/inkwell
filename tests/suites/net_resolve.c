...
static void probe_record(void *userdata, const struct inkwell_resolve_result *result) {
...
INKWELL_TEST_CASE(resolve_literal_needs_no_child, unit) {
    struct sockaddr_storage address;
    socklen_t address_len = 0;

    if (!inkwell_resolve_literal("192.168.1.50", 4403U, &address, &address_len)) {
    ...
    if (!inkwell_resolve_literal("fd00::1", 4404U, &address, &address_len)) {
    ...
    if (inkwell_resolve_literal("device.local", 4403U, &address, &address_len)) {
    ...
    if (inkwell_resolve_literal("192.168.1.50x", 4403U, &address, &address_len)) {
...
INKWELL_TEST_CASE(resolve_finds_a_name, unit) {
    ...
    if (inkwell_resolve_start(&resolve, "localhost", 4403U, probe_record, &probe, 0U) != 0) {
        record_failure(test_name, "the lookup did not start");
        goto cleanup;
    }
    if (!inkwell_resolve_busy(&resolve)) {
    ...
    if (probe.outcome != INKWELL_RESOLVE_OK) {
        /*
...
INKWELL_TEST_CASE(resolve_reports_an_unknown_name, unit) {
    ...
    if (inkwell_resolve_start(&resolve, "localhost", 4403U, probe_record, &working, 0U) != 0 ||
        !pump_until_done(&loop, &resolve, &working)) {
        record_failure(test_name, "the precondition lookup never reported");
        goto cleanup;
    }
    const bool resolver_works = working.outcome == INKWELL_RESOLVE_OK;

    struct resolve_probe probe;
    memset(&probe, 0, sizeof probe);

    /* `.invalid` is reserved by RFC 6761 precisely so that it never resolves anywhere. */
    if (inkwell_resolve_start(&resolve, "inkwell-nothing-here.invalid", 4403U, probe_record, &probe,
                              0U) != 0) {
        record_failure(test_name, "the lookup did not start");
        goto cleanup;
    }
    ...
    if (resolver_works && probe.outcome != INKWELL_RESOLVE_NOT_FOUND) {
...
INKWELL_TEST_CASE(resolve_refuses_a_second_lookup, unit) {
    struct inkwell_loop loop;
    if (inkwell_loop_init(&loop) != 0) {
        record_failure(test_name, "the loop did not start");
        return;
    ...
    if (inkwell_resolve_start(&resolve, "localhost", 4403U, probe_record, &probe, 0U) != -EBUSY) {
    ...
    for (unsigned turn = 0U; turn < 20U; ++turn) {
        ...
        inkwell_resolve_tick(&resolve, (uint64_t)turn * 10U);
    }
    if (probe.calls != 0U) {
...
INKWELL_TEST_CASE(resolve_without_a_loop_is_unavailable, unit) {
    struct inkwell_resolve resolve;
    (void)inkwell_resolve_init(&resolve, NULL);
    ...
    struct resolve_probe probe;
    memset(&probe, 0, sizeof probe);
    if (inkwell_resolve_start(&resolve, "localhost", 4403U, probe_record, &probe, 0U) != -ENOTSUP) {
    ...
    if (inkwell_resolve_start(&resolve, "", 4403U, probe_record, &probe, 0U) != -EINVAL ||
        inkwell_resolve_start(&resolve, "localhost", 4403U, NULL, &probe, 0U) != -EINVAL) {
