...
static bool log_file_tempdir(char *template_path) {
    return mkdtemp(template_path) != NULL;
}

INKWELL_TEST_CASE(log_file_under_the_cap_is_left_alone, unit) {
    char dir[] = "/tmp/inkwell_log_smallXXXXXX";
    INKWELL_TEST_FAIL_IF(!log_file_tempdir(dir), "mkdtemp failed");

    char path[256];
    snprintf(path, sizeof path, "%s/Demo.txt", dir);
    const long written = log_file_write_lines(path, 64U, 80U);
    INKWELL_TEST_FAIL_IF_CLEANUP(written <= 0, inkwell_test_remove_tree(dir),
                                 "could not write a log");

    const long reclaimed = inkwell_log_file_compact(path);
    const long after = log_file_size(path);
    inkwell_test_remove_tree(dir);

    INKWELL_TEST_FAIL_IF(reclaimed != 0L, "a log under the cap reported work it did not do");
    INKWELL_TEST_FAIL_IF(after != written, "a log under the cap was rewritten anyway");
    record_success(test_name);
}

INKWELL_TEST_CASE(log_file_over_the_cap_is_cut_to_its_newest_lines, unit) {
    char dir[] = "/tmp/inkwell_log_bigXXXXXX";
    INKWELL_TEST_FAIL_IF(!log_file_tempdir(dir), "mkdtemp failed");
    ...
    const unsigned lines = (unsigned)((INKWELL_LOG_FILE_MAX_BYTES / width) * 2U);
    const long written = log_file_write_lines(path, lines, width);
    INKWELL_TEST_FAIL_IF_CLEANUP(written <= (long)INKWELL_LOG_FILE_MAX_BYTES,
                                 inkwell_test_remove_tree(dir),
    ...
    INKWELL_TEST_FAIL_IF(reclaimed <= 0L, "an oversized log reported nothing reclaimed");
    INKWELL_TEST_FAIL_IF(after > (long)INKWELL_LOG_FILE_KEEP_BYTES,
                         "the log was not cut back to the retained window");
    INKWELL_TEST_FAIL_IF(after + reclaimed != written,
                         "reclaimed does not account for the shrinkage");
    INKWELL_TEST_FAIL_IF(!kept_newest, "the newest line did not survive the compaction");
    INKWELL_TEST_FAIL_IF(!dropped_oldest, "the oldest line survived a compaction that should have "
                                          "cut it");
    INKWELL_TEST_FAIL_IF(!read_first, "the compacted log had no first line");
    ...
    INKWELL_TEST_FAIL_IF(strncmp(first, "line-", 5U) != 0,
                         "the compacted log begins part-way through a line");
    record_success(test_name);
}

INKWELL_TEST_CASE(log_file_keeps_the_inode_so_an_open_writer_follows, unit) {
    char dir[] = "/tmp/inkwell_log_inodeXXXXXX";
    INKWELL_TEST_FAIL_IF(!log_file_tempdir(dir), "mkdtemp failed");
    ...
    const unsigned width = 80U;
    const unsigned lines = (unsigned)((INKWELL_LOG_FILE_MAX_BYTES / width) * 2U);
    INKWELL_TEST_FAIL_IF_CLEANUP(log_file_write_lines(path, lines, width) <= 0,
                                 inkwell_test_remove_tree(dir), "could not write a log");
    ...
    INKWELL_TEST_FAIL_IF(!statted || !restatted,
                         "could not stat the log either side of a compaction");
    INKWELL_TEST_FAIL_IF(reclaimed <= 0L, "an oversized log reported nothing reclaimed");
    INKWELL_TEST_FAIL_IF(before.st_ino != after.st_ino,
                         "the compaction replaced the inode, so an open tee would follow the wrong "
                         "file");
    INKWELL_TEST_FAIL_IF(!scratch_gone, "the compaction left a stray file beside the log");
    record_success(test_name);
}

INKWELL_TEST_CASE(log_file_leaves_an_appending_writer_correct, unit) {
    char dir[] = "/tmp/inkwell_log_appendXXXXXX";
    INKWELL_TEST_FAIL_IF(!log_file_tempdir(dir), "mkdtemp failed");
    ...
    INKWELL_TEST_FAIL_IF_CLEANUP(writer < 0, inkwell_test_remove_tree(dir),
                                 "could not open an appender");
    ...
    INKWELL_TEST_FAIL_IF(reclaimed <= 0L, "an oversized log reported nothing reclaimed");
    INKWELL_TEST_FAIL_IF(put != (ssize_t)(sizeof after_line - 1U),
                         "the appender could not write after the compaction");
    INKWELL_TEST_FAIL_IF(!landed, "the appender's line did not reach the compacted log");
    ...
    INKWELL_TEST_FAIL_IF(size > (long)INKWELL_LOG_FILE_KEEP_BYTES + (long)sizeof after_line,
                         "the appender wrote past the end, which means it kept its old offset");
    record_success(test_name);
}

INKWELL_TEST_CASE(log_file_absent_is_not_an_error_and_is_not_created, unit) {
    char dir[] = "/tmp/inkwell_log_absentXXXXXX";
    INKWELL_TEST_FAIL_IF(!log_file_tempdir(dir), "mkdtemp failed");
    ...
    INKWELL_TEST_FAIL_IF(reclaimed != 0L,
                         "a missing log was reported as a failure or as work done");
    INKWELL_TEST_FAIL_IF(created, "compacting a missing log created it");
    record_success(test_name);
}

INKWELL_TEST_CASE(log_file_one_enormous_line_is_left_alone, unit) {
    char dir[] = "/tmp/inkwell_log_oneXXXXXX";
    INKWELL_TEST_FAIL_IF(!log_file_tempdir(dir), "mkdtemp failed");
    ...
    const long written = log_file_write_lines(path, 1U, (unsigned)INKWELL_LOG_FILE_MAX_BYTES * 2U);
    INKWELL_TEST_FAIL_IF_CLEANUP(written <= (long)INKWELL_LOG_FILE_MAX_BYTES,
                                 inkwell_test_remove_tree(dir),
                                 "the fixture did not exceed the cap");
    ...
    INKWELL_TEST_FAIL_IF(reclaimed != 0L, "a log with no line boundary reported work it cannot do");
    INKWELL_TEST_FAIL_IF(after != written, "a log with no line boundary was truncated anyway");
    record_success(test_name);
}
 ...
 */
INKWELL_TEST_CASE(log_file_refuses_anything_but_a_regular_file, unit) {
    char dir[] = "/tmp/inkwell_log_fifoXXXXXX";
    INKWELL_TEST_FAIL_IF(!log_file_tempdir(dir), "mkdtemp failed");
    ...
    const long reclaimed = made ? inkwell_log_file_compact(path) : -1L;
    struct stat info;
    const bool still_a_fifo = stat(path, &info) == 0 && S_ISFIFO(info.st_mode);
    inkwell_test_remove_tree(dir);

    INKWELL_TEST_FAIL_IF(!made, "could not create a fifo to stand in for a non-regular log");
    INKWELL_TEST_FAIL_IF(reclaimed != 0L, "a non-regular file was treated as a log to cut back");
    INKWELL_TEST_FAIL_IF(!still_a_fifo, "the compaction replaced a non-regular file");
    record_success(test_name);
}

INKWELL_TEST_CASE(log_file_path_is_derived_from_home, unit) {
    char *const saved_home = getenv("HOME");
    char home_copy[256] = {0};
    if (saved_home != NULL) {
        snprintf(home_copy, sizeof home_copy, "%s", saved_home);
    }
    (void)unsetenv("INKWELL_LOG_FILE");

    /* What launch.sh exports: the pak's own userdata directory. The log is its sibling. */
    (void)setenv("HOME", "/mnt/SDCARD/.userdata/tg5040/Demo", 1);
    char path[INKWELL_LOG_FILE_PATH_MAX];
    const bool derived = inkwell_log_file_default_path(path, sizeof path);
    const bool matched = derived && strcmp(path, "/mnt/SDCARD/.userdata/tg5040/logs/Demo.txt") == 0;

    /* A trailing slash must not make the pak name empty. */
    (void)setenv("HOME", "/mnt/SDCARD/.userdata/tg5040/Demo/", 1);
    char slashed[INKWELL_LOG_FILE_PATH_MAX];
    const bool derived_slashed = inkwell_log_file_default_path(slashed, sizeof slashed);
    const bool matched_slashed =
        derived_slashed && strcmp(slashed, "/mnt/SDCARD/.userdata/tg5040/logs/Demo.txt") == 0;

    /* Nothing to derive from is an answer, not a guess. */
    (void)unsetenv("HOME");
    char unset[INKWELL_LOG_FILE_PATH_MAX];
    const bool derived_unset = inkwell_log_file_default_path(unset, sizeof unset);

    if (home_copy[0] != '\0') {
        (void)setenv("HOME", home_copy, 1);
    }

    INKWELL_TEST_FAIL_IF(!matched,
                         "HOME did not derive the log beside the pak's userdata directory");
    INKWELL_TEST_FAIL_IF(!matched_slashed, "a trailing slash on HOME broke the derivation");
    INKWELL_TEST_FAIL_IF(derived_unset, "a path was claimed with no HOME to derive it from");
    INKWELL_TEST_FAIL_IF(unset[0] != '\0', "a failed derivation left something in the buffer");
    record_success(test_name);
}
 ...
 */
INKWELL_TEST_CASE(log_file_path_refuses_a_home_that_is_not_the_pak_userdata_dir, unit) {
    char *const saved_home = getenv("HOME");
    char home_copy[256] = {0};
    if (saved_home != NULL) {
        snprintf(home_copy, sizeof home_copy, "%s", saved_home);
    ...
    static const char *const strangers[] = {
        ...
        "/mnt/SDCARD/userdata/tg5040/Demo", /* userdata, but not the dot-directory */
        "/mnt/SDCARD/.userdata/Demo",       /* no platform component between them */
        "/tmp",                             /* one component */
    };
    ...
    (void)setenv("HOME", "/mnt/SDCARD/.userdata/tg5040/Demo", 1);
    char device[INKWELL_LOG_FILE_PATH_MAX];
    const bool device_ok = inkwell_log_file_default_path(device, sizeof device) &&
                           strcmp(device, "/mnt/SDCARD/.userdata/tg5040/logs/Demo.txt") == 0;

    /* And an explicit override is not subject to the shape at all. */
    (void)setenv("HOME", "/home/user", 1);
    (void)setenv("INKWELL_LOG_FILE", "/tmp/anywhere.txt", 1);
    char override[INKWELL_LOG_FILE_PATH_MAX];
    const bool override_ok = inkwell_log_file_default_path(override, sizeof override) &&
    ...
    INKWELL_TEST_FAIL_IF(derived_any && offender != NULL,
                         "a HOME outside the pak's userdata directory still derived a log path");
    INKWELL_TEST_FAIL_IF(!device_ok, "the real device layout stopped deriving its log");
    INKWELL_TEST_FAIL_IF(!override_ok, "INKWELL_LOG_FILE was made subject to the HOME shape");
    record_success(test_name);
}

INKWELL_TEST_CASE(log_file_path_override_wins, unit) {
    char *const saved_home = getenv("HOME");
    char home_copy[256] = {0};
    if (saved_home != NULL) {
        snprintf(home_copy, sizeof home_copy, "%s", saved_home);
    ...
    (void)setenv("HOME", "/mnt/SDCARD/.userdata/tg5040/Demo", 1);
    (void)setenv("INKWELL_LOG_FILE", "/tmp/somewhere-else.txt", 1);
    char path[INKWELL_LOG_FILE_PATH_MAX];
    const bool derived = inkwell_log_file_default_path(path, sizeof path);
