...
static void feed(struct parsed *out, const char *text, size_t len, bool head_request, size_t step,
    ...
    inkwell_http_response_init(&out->response, head_request);
    ...
    while (at < len) {
        ...
        while (used < window) {
            ...
            size_t body_len = 0U;
            const size_t consumed = inkwell_http_response_feed(&out->response, in + at + used,
    ...
    if (close) {
        out->finished = inkwell_http_response_finish(&out->response);
    }
}
...
static bool parse(const char *test_name, struct parsed *whole, const char *text, bool head_request,
    ...
    if (whole->response.status != bytewise.response.status ||
        ...
        inkwell_http_response_done(&whole->response) !=
            inkwell_http_response_done(&bytewise.response)) {
...
static bool header_is(const struct inkwell_http_response *response, const char *name,
                      const char *want) {
    const char *value = NULL;
    size_t len = 0U;
    return inkwell_http_response_header(response, name, &value, &len) && len == strlen(want) &&
           memcmp(value, want, len) == 0;
}

/* ------------------------------------------------------------------ URLs */

INKWELL_TEST_CASE(http_url_parses_the_parts, unit) {
    struct inkwell_http_url url;
    INKWELL_TEST_FAIL_IF(
        !inkwell_http_url_parse("https://api.github.com/repos/a/b/releases?x=1#frag", &url),
        "an ordinary https URL should parse");
    INKWELL_TEST_FAIL_IF(!url.tls || url.port != 443U || strcmp(url.host, "api.github.com") != 0,
                         "https should default to 443 and keep the host");
    INKWELL_TEST_FAIL_IF(strcmp(url.target, "/repos/a/b/releases?x=1") != 0,
                         "the target is path and query, never the fragment");

    INKWELL_TEST_FAIL_IF(!inkwell_http_url_parse("HTTP://example.org:8080", &url),
                         "a scheme is case-insensitive and a path is optional");
    INKWELL_TEST_FAIL_IF(url.tls || url.port != 8080U || strcmp(url.target, "/") != 0,
                         "an explicit port is kept and an empty path is '/'");

    INKWELL_TEST_FAIL_IF(!inkwell_http_url_parse("https://example.org?q", &url) ||
                             strcmp(url.target, "/?q") != 0,
                         "a query with no path still gets a '/'");
    INKWELL_TEST_FAIL_IF(!inkwell_http_url_parse("https://[fd00::1]:8443/x", &url) ||
                             strcmp(url.host, "fd00::1") != 0 || url.port != 8443U,
                         "a v6 literal loses its brackets and keeps its port");
    INKWELL_TEST_FAIL_IF(!inkwell_http_url_parse("https://example.org:/x", &url) ||
                             url.port != 443U,
                         "an empty port is the default one");
    record_success(test_name);
}

INKWELL_TEST_CASE(http_url_refuses_what_it_cannot_send, unit) {
    ...
    for (size_t i = 0U; i < sizeof k_bad / sizeof k_bad[0]; ++i) {
        struct inkwell_http_url url;
        if (inkwell_http_url_parse(k_bad[i], &url)) {
            char why[160];
    ...
    char longest[INKWELL_HTTP_URL_MAX + 32U];
    memcpy(longest, "https://example.org/", 20U);
    memset(longest + 20, 'a', sizeof longest - 21U);
    longest[sizeof longest - 1U] = '\0';
    struct inkwell_http_url url;
    INKWELL_TEST_FAIL_IF(inkwell_http_url_parse(longest, &url),
                         "a URL that does not fit is refused");
    record_success(test_name);
}
...
INKWELL_TEST_CASE(http_url_resolves_a_location, unit) {
    ...
    INKWELL_TEST_FAIL_IF(
        !inkwell_http_url_parse("https://github.com/o/r/releases/download/v1/a.bin?x=1", &base),
        "the base should parse");

    struct inkwell_http_url out;
    INKWELL_TEST_FAIL_IF(
        !inkwell_http_url_resolve(
            &base,
    ...
    INKWELL_TEST_FAIL_IF(!inkwell_http_url_resolve(&base, "//cdn.example/x", &out) || !out.tls ||
                             strcmp(out.host, "cdn.example") != 0 || strcmp(out.target, "/x") != 0,
                         "a scheme-relative Location keeps the scheme");
    INKWELL_TEST_FAIL_IF(!inkwell_http_url_resolve(&base, "/other", &out) ||
                             strcmp(out.host, "github.com") != 0 ||
                             strcmp(out.target, "/other") != 0,
                         "an absolute path keeps the host");
    INKWELL_TEST_FAIL_IF(!inkwell_http_url_resolve(&base, "b.bin", &out) ||
                             strcmp(out.target, "/o/r/releases/download/v1/b.bin") != 0,
                         "a relative path replaces the last segment and drops the query");
    INKWELL_TEST_FAIL_IF(!inkwell_http_url_resolve(&base, "?y=2", &out) ||
                             strcmp(out.target, "/o/r/releases/download/v1/a.bin?y=2") != 0,
                         "a query-only reference keeps the whole path");

    INKWELL_TEST_FAIL_IF(!inkwell_http_url_resolve(&base, "#part", &out) ||
                             strcmp(out.host, "github.com") != 0 ||
                             strcmp(out.target, "/o/r/releases/download/v1/a.bin?x=1") != 0,
                         "a fragment-only reference is the base, path and query both");

    INKWELL_TEST_FAIL_IF(inkwell_http_url_resolve(&base, "ftp://example.org/x", &out),
                         "a redirect to another scheme is refused");
    INKWELL_TEST_FAIL_IF(inkwell_http_url_resolve(&base, "/a\r\nX-Evil: 1", &out),
                         "a Location with a line break in it is refused");
    INKWELL_TEST_FAIL_IF(inkwell_http_url_resolve(&base, "", &out), "an empty Location is refused");
    record_success(test_name);
}

/* ------------------------------------------------------------------ the request */

INKWELL_TEST_CASE(http_request_is_the_1_1_shape, unit) {
    struct inkwell_http_url url;
    INKWELL_TEST_FAIL_IF(!inkwell_http_url_parse("https://api.github.com/repos/a/b", &url),
                         "parse");
    const char *const headers[] = {"Accept: application/vnd.github+json", "User-Agent: t/1", NULL};
    char out[512];
    int len = inkwell_http_request_format(out, sizeof out, INKWELL_HTTP_GET, &url, headers, 4U);
    static const char k_want[] = "GET /repos/a/b HTTP/1.1\r\n"
                                 ...
                                 "User-Agent: t/1\r\n"
                                 "\r\n";
    INKWELL_TEST_FAIL_IF(len != (int)strlen(k_want) || strcmp(out, k_want) != 0,
                         "a GET should be exactly the request line, Host, Connection and headers");

    INKWELL_TEST_FAIL_IF(!inkwell_http_url_parse("http://[fd00::1]:8080/x", &url), "parse v6");
    len = inkwell_http_request_format(out, sizeof out, INKWELL_HTTP_HEAD, &url, NULL, 0U);
    INKWELL_TEST_FAIL_IF(
...
INKWELL_TEST_CASE(http_request_refuses_a_header_that_is_two, unit) {
    struct inkwell_http_url url;
    INKWELL_TEST_FAIL_IF(!inkwell_http_url_parse("https://example.org/", &url), "parse");
    const char *const smuggled[] = {"Range: bytes=0-1\r\nX-Evil: 1"};
    char out[512];
    INKWELL_TEST_FAIL_IF(
        inkwell_http_request_format(out, sizeof out, INKWELL_HTTP_GET, &url, smuggled, 1U) != -1,
        "a header carrying a line break is refused");
    INKWELL_TEST_FAIL_IF(inkwell_http_request_format(out, 20U, INKWELL_HTTP_GET, &url, NULL, 0U) !=
                             -1,
...
INKWELL_TEST_CASE(http_response_reads_a_content_length_body, unit) {
    ...
    INKWELL_TEST_FAIL_IF(parsed.response.status != 200, "the status should be read");
    INKWELL_TEST_FAIL_IF(parsed.response.framing != INKWELL_HTTP_FRAMING_LENGTH ||
                             parsed.response.content_length != 5U,
                         "Content-Length frames the body");
    INKWELL_TEST_FAIL_IF(!body_is(&parsed, "hello"), "the body is exactly Content-Length bytes");
    INKWELL_TEST_FAIL_IF(!inkwell_http_response_done(&parsed.response), "and then it is done");
    INKWELL_TEST_FAIL_IF(parsed.unconsumed != 8U, "what follows the body is not consumed");
    record_success(test_name);
}
...
INKWELL_TEST_CASE(http_response_stops_after_the_head, unit) {
    static const char k_text[] =
        "HTTP/1.1 302 Found\r\nLocation: /x\r\nContent-Length: 3\r\n\r\nabc";
    static struct inkwell_http_response response;
    inkwell_http_response_init(&response, false);
    const uint8_t *body = NULL;
    size_t body_len = 0U;
    const size_t consumed = inkwell_http_response_feed(&response, (const uint8_t *)k_text,
                                                       strlen(k_text), &body, &body_len);
    INKWELL_TEST_FAIL_IF(consumed != strlen(k_text) - 3U || body_len != 0U,
                         "the first feed should end exactly at the end of the head");
    INKWELL_TEST_FAIL_IF(!inkwell_http_response_head_done(&response) || response.status != 302,
                         "and the head should be readable then");
    INKWELL_TEST_FAIL_IF(!header_is(&response, "location", "/x"), "Location should be findable");
    record_success(test_name);
}

INKWELL_TEST_CASE(http_response_reads_a_chunked_body, unit) {
    static struct parsed parsed;
    if (!parse(test_name, &parsed,
    ...
    INKWELL_TEST_FAIL_IF(parsed.response.framing != INKWELL_HTTP_FRAMING_CHUNKED,
                         "chunked framing");
    INKWELL_TEST_FAIL_IF(!body_is(&parsed, "hello, world0123456789"),
                         "the body is the chunks' data, without sizes, extensions or CRLFs");
    INKWELL_TEST_FAIL_IF(!inkwell_http_response_done(&parsed.response),
                         "a zero chunk and the end of the trailers finish it");
    INKWELL_TEST_FAIL_IF(parsed.response.body_received != 22U,
                         "body_received counts decoded bytes");
    record_success(test_name);
}

INKWELL_TEST_CASE(http_response_takes_bare_line_feeds, unit) {
    static struct parsed parsed;
    if (!parse(test_name, &parsed, "HTTP/1.1 200 OK\nTransfer-Encoding: chunked\n\n3\nabc\n0\n\n",
               false, false)) {
        return;
    }
    INKWELL_TEST_FAIL_IF(!body_is(&parsed, "abc") || !inkwell_http_response_done(&parsed.response),
                         "LF alone is a line ending a recipient may accept");
    record_success(test_name);
}

INKWELL_TEST_CASE(http_response_reads_until_close, unit) {
    static struct parsed parsed;
    if (!parse(test_name, &parsed, "HTTP/1.0 200 OK\r\n\r\nall of it", false, true)) {
        return;
    }
    INKWELL_TEST_FAIL_IF(parsed.response.framing != INKWELL_HTTP_FRAMING_UNTIL_CLOSE,
                         "no length and no chunking is a body that ends with the connection");
    INKWELL_TEST_FAIL_IF(!parsed.finished || !inkwell_http_response_done(&parsed.response),
                         "for which the close is the end");
    INKWELL_TEST_FAIL_IF(!body_is(&parsed, "all of it"), "and everything before it is body");
    record_success(test_name);
}

INKWELL_TEST_CASE(http_response_has_no_body_where_none_is_sent, unit) {
    static struct parsed parsed;
    ...
    INKWELL_TEST_FAIL_IF(parsed.response.framing != INKWELL_HTTP_FRAMING_NONE || !parsed.finished,
                         "a HEAD reply is done at the end of its head");
    INKWELL_TEST_FAIL_IF(strstr(parsed.response.head, "Content-Length: 1048576") == NULL,
...
INKWELL_TEST_CASE(http_response_skips_an_interim_answer, unit) {
    static struct parsed parsed;
    if (!parse(test_name, &parsed,
    ...
    INKWELL_TEST_FAIL_IF(parsed.response.status != 200 || !body_is(&parsed, "ok"),
                         "1xx answers are passed over for the one that follows");
    INKWELL_TEST_FAIL_IF(strstr(parsed.response.head, "Early Hints") != NULL,
                         "and the head kept is only the final one");
    record_success(test_name);
}

INKWELL_TEST_CASE(http_response_transfer_encoding_beats_length, unit) {
    static struct parsed parsed;
    if (!parse(test_name, &parsed,
               "HTTP/1.1 200 OK\r\nContent-Length: 100\r\nTransfer-Encoding: Chunked\r\n\r\n"
               "2\r\nhi\r\n0\r\n\r\n",
    ...
    INKWELL_TEST_FAIL_IF(parsed.response.framing != INKWELL_HTTP_FRAMING_CHUNKED ||
                             !body_is(&parsed, "hi") ||
                             !inkwell_http_response_done(&parsed.response),
                         "with both, Transfer-Encoding frames the body (RFC 9112 6.3)");
    record_success(test_name);
}

INKWELL_TEST_CASE(http_response_headers_are_found_without_case, unit) {
    static struct parsed parsed;
    if (!parse(test_name, &parsed,
               "HTTP/1.1 200 OK\r\nX-Empty:\r\nETag:   \"abc\"  \r\nContent-Length: 0\r\n\r\n",
               false, false)) {
    ...
    INKWELL_TEST_FAIL_IF(!header_is(&parsed.response, "etag", "\"abc\""),
                         "a value is found by any case of its name, trimmed");
    INKWELL_TEST_FAIL_IF(!header_is(&parsed.response, "x-empty", ""), "an empty value is a value");
    const char *value = NULL;
    size_t len = 0U;
    INKWELL_TEST_FAIL_IF(inkwell_http_response_header(&parsed.response, "location", &value, &len),
...
struct refusal {
    const char *text;
    enum inkwell_http_error want;
    const char *why;
};

INKWELL_TEST_CASE(http_response_refuses_ambiguous_framing, unit) {
    static const struct refusal k_cases[] = {
        {"HTTP/2 200\r\n\r\n", INKWELL_HTTP_ERR_STATUS_LINE, "HTTP/2 is not spoken here"},
        {"ICY 200 OK\r\n\r\n", INKWELL_HTTP_ERR_STATUS_LINE, "not an HTTP status line"},
        {"HTTP/1.1 20\r\n\r\n", INKWELL_HTTP_ERR_STATUS_LINE, "a two-digit status"},
        {"HTTP/1.1 200OK\r\n\r\n", INKWELL_HTTP_ERR_STATUS_LINE, "no space after the status"},
        {"HTTP/1.1 101 Switching Protocols\r\nUpgrade: h2c\r\n\r\n", INKWELL_HTTP_ERR_UPGRADE,
         "a switch that was never asked for"},
        {"HTTP/1.1 200 OK\r\nX-A: 1\r\n folded\r\n\r\n", INKWELL_HTTP_ERR_HEADER, "a folded line"},
        {"HTTP/1.1 200 OK\r\nContent-Length : 5\r\n\r\nhello", INKWELL_HTTP_ERR_HEADER,
         "whitespace before the colon"},
        {"HTTP/1.1 200 OK\r\nno colon here\r\n\r\n", INKWELL_HTTP_ERR_HEADER,
         "a line with no colon"},
        {"HTTP/1.1 200 OK\r\nContent-Length: 5\r\nContent-Length: 6\r\n\r\nhello!",
         INKWELL_HTTP_ERR_LENGTH, "two lengths that disagree"},
        {"HTTP/1.1 200 OK\r\nContent-Length: 5, 5\r\n\r\nhello", INKWELL_HTTP_ERR_LENGTH,
         "a length that is a list"},
        {"HTTP/1.1 200 OK\r\nContent-Length: -1\r\n\r\n", INKWELL_HTTP_ERR_LENGTH,
         "a signed length"},
        {"HTTP/1.1 200 OK\r\nContent-Length: 99999999999999999999\r\n\r\n", INKWELL_HTTP_ERR_LENGTH,
         "a length past 64 bits"},
        {"HTTP/1.1 200 OK\r\nTransfer-Encoding: gzip, chunked\r\n\r\n", INKWELL_HTTP_ERR_ENCODING,
         "a coding this never asked for"},
        {"HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\nTransfer-Encoding: chunked\r\n\r\n",
         INKWELL_HTTP_ERR_ENCODING, "chunked twice"},
        {"HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\nzz\r\n", INKWELL_HTTP_ERR_CHUNK,
         "a size that is not hex"},
        {"HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\n3x\r\n", INKWELL_HTTP_ERR_CHUNK,
         "junk after a size"},
        {"HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\n11111111111111111\r\n",
         INKWELL_HTTP_ERR_CHUNK, "a size past 64 bits"},
        {"HTTP/1.1 200 OK\r\nTransfer-Encoding: chunked\r\n\r\n3\r\nabcX\r\n",
         INKWELL_HTTP_ERR_CHUNK, "a chunk longer than its size"},
    };
    for (size_t i = 0U; i < sizeof k_cases / sizeof k_cases[0]; ++i) {
        ...
        if (!inkwell_http_response_failed(&parsed.response) ||
            parsed.response.error != k_cases[i].want) {
            char why[200];
            snprintf(why, sizeof why, "should refuse %s as \"%s\", got \"%s\"", k_cases[i].why,
                     inkwell_http_error_name(k_cases[i].want),
                     inkwell_http_error_name(parsed.response.error));
            record_failure(test_name, why);
            return;
        }
    }
...
INKWELL_TEST_CASE(http_response_says_when_it_was_cut_short, unit) {
    ...
    for (size_t i = 0U; i < sizeof k_cut / sizeof k_cut[0]; ++i) {
        ...
        INKWELL_TEST_FAIL_IF(parsed.finished || parsed.response.error != INKWELL_HTTP_ERR_TRUNCATED,
                             "a close before the framing's end is a truncation, not an end");
    }
    record_success(test_name);
}

INKWELL_TEST_CASE(http_response_bounds_the_head, unit) {
    static char text[INKWELL_HTTP_HEAD_MAX + 64U];
    ...
    INKWELL_TEST_FAIL_IF(parsed.response.error != INKWELL_HTTP_ERR_HEAD_TOO_LARGE,
