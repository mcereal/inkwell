...
INKWELL_TEST_CASE(json_reads_an_object, unit) {
    static const char k_document[] = "  { \"name\" : \"T114\" , \"model\": 69, \"dfu\" : true ,\n"
                                     "    \"tags\": [\"a\", \"b\"], \"nested\": {\"x\": {} } }";
    struct inkwell_json json;
    inkwell_json_init(&json, k_document, 0U);
    INKWELL_TEST_FAIL_IF(!inkwell_json_enter_object(&json), "the document is an object");
    ...
    while (inkwell_json_next_key(&json, key, sizeof key)) {
        if (strcmp(key, "name") == 0) {
            INKWELL_TEST_FAIL_IF(!inkwell_json_read_string(&json, name, sizeof name),
                                 "name is a string");
            seen++;
        } else if (strcmp(key, "model") == 0) {
            INKWELL_TEST_FAIL_IF(!inkwell_json_read_u64(&json, &model), "model is a number");
            seen++;
        } else if (strcmp(key, "dfu") == 0) {
            INKWELL_TEST_FAIL_IF(!inkwell_json_read_bool(&json, &dfu), "dfu is a bool");
            seen++;
        } else {
            INKWELL_TEST_FAIL_IF(!inkwell_json_skip_value(&json), "an unread value should skip");
        }
    }
    INKWELL_TEST_FAIL_IF(seen != 3U, "every key should have been offered exactly once");
    INKWELL_TEST_FAIL_IF(strcmp(name, "T114") != 0 || model != 69U || !dfu,
 ...
 */
INKWELL_TEST_CASE(json_ignores_structure_inside_a_string, unit) {
    static const char k_document[] =
        "{\"notes\": \"reverted the change that set \\\"id\\\": \\\"v9.9.9\\\" and a {brace\","
        " \"id\": \"v2.7.26\"}";
    struct inkwell_json json;
    inkwell_json_init(&json, k_document, 0U);
    INKWELL_TEST_FAIL_IF(!inkwell_json_object_find(&json, "id"), "the real id should be found");

    char id[16];
    INKWELL_TEST_FAIL_IF(!inkwell_json_read_string(&json, id, sizeof id), "the id is a string");
    INKWELL_TEST_FAIL_IF(strcmp(id, "v2.7.26") != 0, "a decoy inside a note is not a key");
    record_success(test_name);
}

INKWELL_TEST_CASE(json_unescapes_and_truncates_strings, unit) {
    ...
    INKWELL_TEST_FAIL_IF(strcmp(out, "a\"b\\c\n") != 0, "the simple escapes should be unescaped");

    INKWELL_TEST_FAIL_IF(!inkwell_json_next_element(&json) ||
                             !inkwell_json_read_string(&json, out, sizeof out),
                         "the second string should read");
    INKWELL_TEST_FAIL_IF(strcmp(out, "\xc3\xa9"
                                     "A") != 0,
    ...
    INKWELL_TEST_FAIL_IF(strcmp(out, "?lone") != 0, "half a surrogate pair is not a character");
    ...
    INKWELL_TEST_FAIL_IF(strlen(out) != sizeof out - 1U, "it should fill the buffer and terminate");
    INKWELL_TEST_FAIL_IF(inkwell_json_next_element(&json),
                         "the array should end after four elements");
    record_success(test_name);
}
 ...
 * keeping.
 */
INKWELL_TEST_CASE(json_requires_separators_between_members, unit) {
    struct inkwell_json json;
    char key[8];
    char value[8];

    inkwell_json_init(&json, "{\"a\": \"one\" \"b\": \"two\"}", 0U);
    INKWELL_TEST_FAIL_IF(!inkwell_json_enter_object(&json), "the document is an object");
    INKWELL_TEST_FAIL_IF(!inkwell_json_next_key(&json, key, sizeof key) || strcmp(key, "a") != 0,
                         "the first member needs no comma before it");
    INKWELL_TEST_FAIL_IF(!inkwell_json_read_string(&json, value, sizeof value),
                         "and reads normally");
    INKWELL_TEST_FAIL_IF(inkwell_json_next_key(&json, key, sizeof key),
                         "the second, with no comma before it, is not a member");

    inkwell_json_init(&json, "[1 2]", 0U);
    INKWELL_TEST_FAIL_IF(!inkwell_json_enter_array(&json), "the document is an array");
    INKWELL_TEST_FAIL_IF(!inkwell_json_next_element(&json) || !inkwell_json_skip_value(&json),
                         "the first element needs no comma before it");
    INKWELL_TEST_FAIL_IF(inkwell_json_next_element(&json), "and the second is not an element");
    ...
    inkwell_json_init(&json, "{}", 0U);
    INKWELL_TEST_FAIL_IF(!inkwell_json_enter_object(&json), "an empty object is an object");
    INKWELL_TEST_FAIL_IF(inkwell_json_next_key(&json, key, sizeof key), "and holds no key");

    inkwell_json_init(&json, "[[], [{\"a\": 1}, 2], 3]", 0U);
    INKWELL_TEST_FAIL_IF(!inkwell_json_enter_array(&json), "the document is an array");
    unsigned elements = 0U;
    while (inkwell_json_next_element(&json)) {
        INKWELL_TEST_FAIL_IF(!inkwell_json_skip_value(&json), "every element should skip");
        elements++;
    }
    INKWELL_TEST_FAIL_IF(elements != 3U, "three elements, however deeply the middle one nests");
    record_success(test_name);
}
...
INKWELL_TEST_CASE(json_refuses_truncated_input, unit) {
    ...
    for (size_t i = 0; i < sizeof k_broken / sizeof k_broken[0]; ++i) {
        struct inkwell_json json;
        inkwell_json_init(&json, k_broken[i], 0U);
        ...
        char value[32];
        if (inkwell_json_object_find(&json, "a")) {
            (void)inkwell_json_read_string(&json, value, sizeof value);
        }
    }

    /* And the depth limit, which is the one refusal with a number behind it. */
    char deep[2U * INKWELL_JSON_MAX_DEPTH + 8U];
    size_t at = 0U;
    deep[at++] = '{';
    deep[at++] = '"';
    deep[at++] = 'a';
    deep[at++] = '"';
    deep[at++] = ':';
    for (size_t i = 0; i < INKWELL_JSON_MAX_DEPTH + 2U; ++i) {
        deep[at++] = '[';
    }
    deep[at] = '\0';
    struct inkwell_json json;
    inkwell_json_init(&json, deep, 0U);
    INKWELL_TEST_FAIL_IF(!inkwell_json_object_find(&json, "a"), "the key itself is well formed");
    INKWELL_TEST_FAIL_IF(inkwell_json_skip_value(&json),
                         "a value nested past the limit should refuse");
    record_success(test_name);
}
