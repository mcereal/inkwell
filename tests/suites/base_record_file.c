...
INKWELL_TEST_CASE(record_file_discards_torn_tail, unit) {
    FILE *file = tmpfile();
    INKWELL_TEST_FAIL_IF(file == NULL, "could not create scratch file");
    fputs("keep=ok\ntorn=partial", file);
    rewind(file);
    ...
    const int result = inkwell_record_read(file, line, sizeof line, visit_record, &fixture);
    fclose(file);
    INKWELL_TEST_FAIL_IF(result != 0 || fixture.seen != 1U,
                         "an unterminated final line became a record");
    record_success(test_name);
}

INKWELL_TEST_CASE(record_file_accepts_full_buffer_line, unit) {
    FILE *file = tmpfile();
    INKWELL_TEST_FAIL_IF(file == NULL, "could not create scratch file");
    fputs("a=1234567890123\n", file);
    rewind(file);
    char line[16];
    struct record_fixture fixture = {0};
    const int result = inkwell_record_read(file, line, sizeof line, visit_record, &fixture);
    fclose(file);
    INKWELL_TEST_FAIL_IF(result != 0 || fixture.seen != 1U,
                         "a complete line at the buffer boundary was discarded");
    record_success(test_name);
}
