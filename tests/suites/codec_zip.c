...
static bool zip_open(const char *name, uint64_t zip_size, struct zip_fixture *out) {
    size_t len = 0U;
    char *const bytes = inkwell_test_data_read(name, &len);
...
static void zip_close(struct zip_fixture *fixture) {
    free(fixture->window);
}
 ...
 */
INKWELL_TEST_CASE(zip_reads_a_release_tail, unit) {
    struct zip_fixture zip;
    INKWELL_TEST_FAIL_IF(!zip_open("zip_tail_nrf52840_2.7.26.bin", ZIP_SIZE_2_7_26, &zip),
                         "the 2.7.26 tail window should be readable");

    struct inkwell_zip_end end;
    const bool found = inkwell_zip_find_end(zip.window, zip.len, zip.offset, &end);
    INKWELL_TEST_FAIL_IF_CLEANUP(!found, zip_close(&zip),
                                 "the end-of-central-directory record is in the window");
    INKWELL_TEST_FAIL_IF_CLEANUP(end.entries != 139U, zip_close(&zip),
                                 "2.7.26's nrf52840 zip holds 139 members");
    INKWELL_TEST_FAIL_IF_CLEANUP(end.central_size != 16316U, zip_close(&zip),
    ...
    const uint8_t *const central = inkwell_zip_central_slice(&end, zip.window, zip.len, zip.offset);
    INKWELL_TEST_FAIL_IF_CLEANUP(central == NULL, zip_close(&zip),
                                 "the directory fits inside the tail window");

    struct inkwell_zip_entry entry;
    const enum inkwell_zip_search member =
        inkwell_zip_find_member(central, end.central_size, end.entries,
                                "firmware-heltec-mesh-node-t114-2.7.26.54e0d8d.uf2", &entry);
    INKWELL_TEST_FAIL_IF_CLEANUP(member != INKWELL_ZIP_FOUND, zip_close(&zip),
                                 "the T114's image should be in the directory");
    INKWELL_TEST_FAIL_IF_CLEANUP(entry.compressed_size != 517956U ||
                                     entry.uncompressed_size != 1467392U,
                                 zip_close(&zip), "both sizes come off the central directory");
    INKWELL_TEST_FAIL_IF_CLEANUP(entry.crc32 != 3175933648U, zip_close(&zip),
                                 "and so does the CRC the inflate is checked against");
    INKWELL_TEST_FAIL_IF_CLEANUP(entry.method != INKWELL_ZIP_METHOD_DEFLATE, zip_close(&zip),
                                 "release members are deflated");
    INKWELL_TEST_FAIL_IF_CLEANUP(entry.local_header_offset != 33671990ULL, zip_close(&zip),
                                 "the local header is where the served file has it");
    /* Flat at this release. The next case is the same file one release later. */
    INKWELL_TEST_FAIL_IF_CLEANUP(strchr(entry.name, '/') != NULL, zip_close(&zip),
 ...
 */
INKWELL_TEST_CASE(zip_reads_a_prefixed_member, unit) {
    struct zip_fixture zip;
    INKWELL_TEST_FAIL_IF(!zip_open("zip_tail_nrf52840_2.8.0.bin", ZIP_SIZE_2_8_0, &zip),
                         "the 2.8.0 tail window should be readable");

    struct inkwell_zip_end end;
    INKWELL_TEST_FAIL_IF_CLEANUP(!inkwell_zip_find_end(zip.window, zip.len, zip.offset, &end),
                                 zip_close(&zip), "2.8.0's record is in the window too");
    INKWELL_TEST_FAIL_IF_CLEANUP(end.entries != 171U || end.central_size != 21358U, zip_close(&zip),
    ...
    INKWELL_TEST_FAIL_IF_CLEANUP((entry.flags & INKWELL_ZIP_FLAG_DATA_DESCRIPTOR) == 0U,
                                 zip_close(&zip),
                                 "2.8.0's members defer their sizes to a data descriptor");
    INKWELL_TEST_FAIL_IF_CLEANUP(
        entry.compressed_size != 561092U || entry.uncompressed_size != 1491968U ||
            entry.crc32 != 2320343302U,
        zip_close(&zip), "which is why all three of those come from the directory");
    zip_close(&zip);
    record_success(test_name);
}
 ...
 */
INKWELL_TEST_CASE(zip_never_matches_a_directory_marker, unit) {
    struct zip_fixture zip;
    INKWELL_TEST_FAIL_IF(!zip_open("zip_tail_nrf52840_2.8.0.bin", ZIP_SIZE_2_8_0, &zip),
                         "the 2.8.0 tail window should be readable");
    struct inkwell_zip_end end;
    ...
    INKWELL_TEST_FAIL_IF_CLEANUP(inkwell_zip_find_member(central, end.central_size, end.entries, "",
                                                         &entry) != INKWELL_ZIP_ABSENT,
                                 zip_close(&zip), "an empty basename matches nothing");
    INKWELL_TEST_FAIL_IF_CLEANUP(inkwell_zip_find_member(central, end.central_size, end.entries,
                                                         "nrf52840/", &entry) != INKWELL_ZIP_ABSENT,
                                 zip_close(&zip),
 ...
 */
INKWELL_TEST_CASE(zip_places_the_data_from_the_local_header, unit) {
    struct inkwell_zip_entry entry;
    memset(&entry, 0, sizeof entry);
    entry.local_header_offset = 33671990ULL;

    /* The 30 fixed bytes as served, name length 49 and extra length 28. */
    uint8_t header[INKWELL_ZIP_LOCAL_HEADER_SIZE];
    ...
    uint64_t start = 0U;
    INKWELL_TEST_FAIL_IF(!inkwell_zip_local_data_start(header, sizeof header, &entry, &start),
                         "a local header should place its data");
    INKWELL_TEST_FAIL_IF(start != 33671990ULL + 30ULL + 49ULL + 28ULL,
                         "the data begins after the header, the name and the extra field");

    header[0] = 0x00U;
    INKWELL_TEST_FAIL_IF(inkwell_zip_local_data_start(header, sizeof header, &entry, &start),
                         "and a header with no signature is refused");
    INKWELL_TEST_FAIL_IF(inkwell_zip_local_data_start(header, 12U, &entry, &start),
 ...
 */
INKWELL_TEST_CASE(zip_says_when_the_directory_is_outside_the_window, unit) {
    struct zip_fixture zip;
    INKWELL_TEST_FAIL_IF(!zip_open("zip_tail_nrf52840_2.7.26.bin", ZIP_SIZE_2_7_26, &zip),
                         "the 2.7.26 tail window should be readable");
    struct inkwell_zip_end end;
    ...
    struct inkwell_zip_end from_small;
    INKWELL_TEST_FAIL_IF_CLEANUP(!inkwell_zip_find_end(shifted, small, shifted_offset, &from_small),
                                 zip_close(&zip), "a smaller window still finds the record");
    INKWELL_TEST_FAIL_IF_CLEANUP(from_small.central_offset != end.central_offset, zip_close(&zip),
                                 "and reports the same directory");
    INKWELL_TEST_FAIL_IF_CLEANUP(
        inkwell_zip_central_slice(&from_small, shifted, small, shifted_offset) != NULL,
 ...
 */
INKWELL_TEST_CASE(zip_finds_the_record_and_not_a_coincidence, unit) {
    struct zip_fixture zip;
    INKWELL_TEST_FAIL_IF(!zip_open("zip_tail_nrf52840_2.7.26.bin", ZIP_SIZE_2_7_26, &zip),
                         "the 2.7.26 tail window should be readable");
    struct inkwell_zip_end end;
    ...
    struct inkwell_zip_end nothing;
    INKWELL_TEST_FAIL_IF_CLEANUP(inkwell_zip_find_end(zip.window, 8U, zip.offset, &nothing),
                                 zip_close(&zip),
 ...
 */
INKWELL_TEST_CASE(zip_tells_a_broken_directory_from_a_missing_member, unit) {
    struct zip_fixture zip;
    INKWELL_TEST_FAIL_IF(!zip_open("zip_tail_nrf52840_2.7.26.bin", ZIP_SIZE_2_7_26, &zip),
                         "the 2.7.26 tail window should be readable");
    struct inkwell_zip_end end;
    ...
    INKWELL_TEST_FAIL_IF_CLEANUP(inkwell_zip_find_member(central, end.central_size, end.entries,
                                                         ...
                                                         &entry) != INKWELL_ZIP_ABSENT,
                                 zip_close(&zip),
                                 "an ESP32 image is not in the nrf52840 zip, and that is absence");
    ...
    INKWELL_TEST_FAIL_IF_CLEANUP(
        inkwell_zip_find_member(central, end.central_size / 2U, end.entries,
                                "firmware-heltec-mesh-node-t114-2.7.26.54e0d8d.uf2",
                                &entry) != INKWELL_ZIP_MALFORMED,
    ...
    INKWELL_TEST_FAIL_IF_CLEANUP(broken == NULL, zip_close(&zip), "out of memory");
    memcpy(broken, central, end.central_size);
    {
        ...
        const enum inkwell_zip_search search =
            inkwell_zip_find_member(broken, end.central_size, end.entries,
                                    "firmware-heltec-mesh-node-t114-2.7.26.54e0d8d.uf2", &entry);
        free(broken);
        INKWELL_TEST_FAIL_IF_CLEANUP(search != INKWELL_ZIP_MALFORMED, zip_close(&zip),
