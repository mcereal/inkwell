 ...
 */
static int inkwell_test_fs_remove_entry(const char *path, const struct stat *info, int type,
                                        struct FTW *level) {
    (void)info;
    (void)level;
...
bool inkwell_test_remove_tree(const char *path) {
    if (path == NULL || path[0] == '\0') {
        return false;
    }

    s_walk_failures = 0;
    if (nftw(path, inkwell_test_fs_remove_entry, MESH_TEST_FS_WALK_FDS, FTW_DEPTH | FTW_PHYS) !=
        0) {
...
bool inkwell_test_remove_matching(const char *dir, const char *prefix) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (snprintf(child, sizeof child, "%s/%s", dir, matches[i]) >= (int)sizeof child) {
            ...
            ok = inkwell_test_remove_tree(child) && ok;
        }
        free(matches[i]);
    }
    free(matches);
