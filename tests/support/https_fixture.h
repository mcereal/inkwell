 ...
 * *.githubusercontent.com) as well as `example.invalid` and 127.0.0.1, and
...
/*
 * A self-signed certificate this server never presents. For a case that needs a trust anchor
 * which does not match, so that "not in the registered roots" can be told apart from "no
 * registered roots at all".
 */
